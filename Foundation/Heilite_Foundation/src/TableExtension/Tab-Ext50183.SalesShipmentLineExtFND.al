tableextension 50183 SalesShipmentLineExtFND extends "Sales Shipment Line"
{
    ...
    // DITW15.00.00.01 DDR 28/01/2008 Correct GetShipments into function InsertInvLineFromShptLine()
    ...
    // DITW15.00.00.01 DDR 27/02/2008 Remove field (replace/reuse field 2034643 "ItemCharge Incl. Price")
    ...
    //                                Added key1 Sumindexfields "Weight","Cubage"
    //                     07/10/2008 Added fields
    //                                  2013722 Duty Tax Type (flowfield)
    // DITW15.00.00.25 DDR 16/10/2008 Added optionstring 'ShippingCost' for field "Item Charge Type"
    ...
    //                                Update function InsertInvLineFromShptLine()
    ...
    //                     04/09/2009 Added functions ShowItemServices()
    // DITW15.00.00.36 DDR 21/12/2009 issue 971 Modified function InsertInvLineFromShptLine() to include item charges per order
    ...
    // DITW15.00.00.37 DDR 20/01/2010 issue 1020 Added transfer fields for function InsertInvLineFromShptLine()
    ...
    //                 DDR 27/04/2011 Added key "Document No.,Sell-to Customer No.,Type,No.,Unit of Measure Code"
    ...
    //                     13/03/2012 DIT-715 #275 Added functions ShowSSCCTrackingLines()
    //                     20/03/2012 DIT-715 #275 Added 'ShowAll' parameter function ShowSSCCTrackingLines()
    ...
    // DITW16.00.00.42 DDR 08/01/2013 DIT-715 #533 Added transfer EMCS fields for function InsertInvLineFromShptLine()
    //                 DDR 27/02/2013 DIT-715 #550 Modified checking on "Packaging Type Code"
    // DITW16.00.00.43 DDR 14/05/2013 DIT-715 #632 Added key "Document No.,Type,No."
    //                 FBL 18/06/2013 DIT-715 #619 Add field 2034920 "Created by Contract Batch Job" (Boolean) to be copied from sales line
    ...
    // FINXL8.00.001 BSA 27/05/2015 #184: added code for creation of invoice lines for different vendor than order + Permissions on tables

    // DITW17.00.01 DDR 13/02/2013 DIT-770 #001 Upgrade
    // DITW17.00.02 DDR 14/05/2013 DIT-715 #632
    ...
    // DITW17.00.02 RPG 05/11/2013 DIT-770 #235 Added Code to flow Shipment Date, Order No. and Order Line No. in Invoice Item Lines
    ...
    // DITW17.10.05 MSF 25/12/2014 DIT-770 #1115 : Order discounts charges are stored with wrong sign in value entry from combined invoice
    // DITW17.10.04 AKH 19/12/2014 DIT-770 #1022 Merge DIT W1 R4 in R5
    // DITW17.10.05 WSA 03/02/2015 DIT-770 #779 Added Event Fields
    //                                          Added code to clear event info
    ...
    // FINXL9.00.000.01 ACH 10/01/2017 : Added field: "Rec. Charge Attach. Line No."
    // FINXL9.00.000.01 AKH 13/01/2017 Commented changes of issue 184
    // FINXL10.00 AKH 24/03/2017 Upgrade to NAV 2017 CU4
    // FINXL10.00 AKH 02/03/2017 NRQ#25695: Adjusted TableRelation for field 2029614 "Rec. Charge Attach. Line No."
    // DITW110.00.09 AKH 12/04/2017 NRQ#24104 Merge from XL NRQ#25695
    // DITW110.00.10 DDR 12/05/2017 NRQ#26584 Fix NAV drop shipment with combine shipment (report) function
    // DITW110.00.10 YHE 08/06/2017 NRQ#26412 UPGRADE NAV 2017 CU7
    // DITW110.00.10 SFI 20/06/2017 BL#15657 (DIT-770 #934) Added new fields 2014071..2014072
    // DITW110.00.10 MSF 07/07/2017 NRQ#16224 Return Registration part 3
    //                                  Added Key : Document No.,Type,Item DDeposit Group Code
    // DITW110.00.11 SFI 30/08/2017 BL#14417 New field
    //                                         2013637 Deposit Value
    // DITW110.00.11 MSF 04/10/2017 NRQ#39012 : Added field Route Planning No.
    ...
    //   # New Fields created: 50012 - Gate Entry No.
    ...
    //   # Code added on function 'CalcVATAmountLines'
    // HEI.07 INC4055655 - CHG2154321 IBM NASTAA02 11.04.2022 # Issue on missing description name on posted sale shipment line
    //   # Code added to Function 'GetCaptionClass'
    // HEI.08 CHG2161264 DEBUSD01 10.11.2022 Shipment KPI Interface
    //   # 50000 - "Document Sub Type" (flowfield)
    // HEI.09 CHG2178940 IBM COSTES04  16.01.2023 # Add Required Freshness field on Customer Card
    //   # Add new field RFreshness Date (min)

    // BC Upgrade SHUKLP03 >> Added Document Subtype code for field 50090.
    ...
    fields
    {
        ...
        modify("Unit Price")
        {
            ...
            //CaptionClass = GetCaptionClass(FIELDNO("Unit Price"));//BC Upgrade SHARMP16 Drink-IT function used.
        }
        ...
        {
            CaptionML = ENU = 'Authorized for Credit Card', FRA = 'Autorisé pour la carte de crédit';
        }
        modify("Job Task No.")
        {
        ...
        //BC Upgrade SHARMP16 begin>> These fields obselete in BC functional query
        ...
        // }
        //BC Upgrade SHARMP16 End<< These fields obselete in BC functional query
        ...
        {
            CalcFormula = Lookup("Sales Shipment Header"."Document Subtype Code FND" where("No." = FIELD("Document No.")));//BC Upgrade SHUKLP03 -- Document Subtype code
            Caption = 'Document Subtype Code';
            Description = 'HEI.08';
            Editable = false;
            FieldClass = FlowField;
            TableRelation = "Document Subtype Code FND".Code where("Report Selection Type" = FILTER(Sales | "Fin.Contract"));//BC Upgrade SHUKLP03 -- Document Subtype code
        }
        ...
        field(50011; "Transport RPM Damage/Loss FND"; Boolean)
        {
            Caption = 'Transporter RPM Damage / Loss';
            Description = 'HEI.04';
        }
        field(50012; "Gate Entry No. FND"; Code[20])
        {
        ...
        //     Description = 'DITW17.10.03 DIT-770 623,HEI.02';
        ...
        // {
        //     CalcFormula = Lookup("Sales Shipment Header"."Route Planning No." where("No." = FIELD("Document No.")));
        ...
        //               DATABASE::"Sales Shipment Line", "Document No.", "ARC No.", true);

        //             SalesShptLine.SETRANGE("Document No.", "Document No.");
        //             SalesShptLine.SETFILTER("Line No.", '<>%1', "Line No.");
        //             SalesShptLine.SETRANGE("ARC No.", "ARC No.");
        //             SalesShptLine.MODIFYALL("Cancellation Reason Type", "Cancellation Reason Type");
        //         end;
        //     end;
        // }
        // field(2014292; "Cancellation Reason Comment"; Boolean)
        // {
        //     CalcFormula = Exist("EMCS Comment Line" where("Table ID" = CONST(111),
        ...
        // {
        //     CalcFormula = Exist("Sales Shipment Line" where("Document No." = FIELD("Document No."),
        ...
        //         if "Loyalty Point Type" <> "Loyalty Point Type"::" " then
        ...
        // }
        // field(2029614; "Rec. Charge Attach. Line No."; Integer)
        // {
        //     Caption = 'Recycle Charge';
    ...
    keys
    {
        ...
        //Bc Upgrade SHARMP16 Begin<< ------------------ Drink-IT Fieldsused
        ...
        // {
        // }//Bc Upgrade SHARMP16 End>> ------------------ Drink-IT Fieldsused
        key(Key50000; "Document No.", "Sell-to Customer No.", Type, "No.", "Unit of Measure Code")
        {
            SumIndexFields = Quantity, "Quantity (Base)";
        }
        //Bc Upgrade SHARMP16 Begin<< ------------------ Drink-IT Fieldsused
        // key(Key7; "Applies-to AAD Trck. Entry No.")
        // {
        // }
        // key(Key8; "Document No.", Type, "No.")
        // {
        //     SumIndexFields = "Delivery Time (sec.)";
        // }
        // key(Key9; "Document No.", Type, "Item DDeposit Group Code")
        // {
        // }
        //Bc Upgrade SHARMP16 End>> ------------------ Drink-IT Fieldsused
    }
    ...
    */
    ...
    EmcsCommentLine.SETRANGE("Table ID",DATABASE::"Sales Shipment Line");
    ...
    */
    ...
    var
        ItemChargeSalesShptLine: Record "Sales Shipment Line";
    ...
    //var
    //>>>> ORIGINAL VALUE:
    //Text000 : ENU=Shipment No. %1:;
    //Variable type has not been exported.
    //>>>> MODIFIED VALUE:
    //Text000 : ENU=Shipment No. %1:;FRA=N° expédition %1 :;
    //Variable type has not been exported.
    ...
    //var
    //>>>> ORIGINAL VALUE:
    //Text001 : ENU=The program cannot find this Sales line.;
    //Variable type has not been exported.
    //>>>> MODIFIED VALUE:
    //Text001 : ENU=The program cannot find this Sales line.;FRA=Le programme ne trouve pas cette ligne vente.;
    //Variable type has not been exported.


    //Unsupported feature: PropertyModification on "ItemNoFieldCaptionTxt(Variable 1007)". Please convert manually.
    ...
    var
        ...
        Text2014410: TextConst ENU = 'Shipment No. %1 %2', FRA = 'N° expédition %1 %2';
        Text2014411: TextConst ENU = 'Sell-to Customer No. %1', FRA = 'N° donneur d''ordre %1';
        Text2014412: TextConst ENU = 'Order No. %1', FRA = 'N° commande %1';
