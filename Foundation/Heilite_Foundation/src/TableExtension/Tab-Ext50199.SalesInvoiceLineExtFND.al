tableextension 50199 SalesInvoiceLineExtFND extends "Sales Invoice Line"
{
    // version NAVW110.0.00.16585,FINXL10.00,DITW110.00.10,HEI.13
    ...
    //                                Added function GetAnySalesShptLines()
    //                                Added fields
    ...
    // DITW15.00.00.36 DDR 21/12/2009 issue 786 Added fields
    ...
    //                                               2014260 LRN No. Series
    //                                               2014265 Product Tax Code
    //                                               2014267 ARC No. Mandatory
    //                                               2014271 Company Tax Warehouse Ref.
    ...
    // DITW19.00.07 MVN 30/12/2015 DIT-770 #001 Upgrade: Set Global GetSalesShptLines
    ...
    // FINXL9.00 RGO 27/06/2016 : Removed local property  from fct GetSalesShptLines
    ...
    // HEI.02 FDD-SLSGAP001 IBM NASTAA02 19.09.2017 # MDM Customer Card
    ...
    // HEI.06 CHG2009225 IBM ISYED01 5/20/2019 # Reduce labor intensive entries in 1day returns process-FDD
    ...
    // HEI.07 FDD-ET-MARAKI POS Interface IBM NASTAA02 21.06.2018 # Maraki POS Interface
    //   # New Field created: 50018 - Suppress POS Interface
    // HEI.08 FDD-SR_HT543a-KSTORE EXPORT IBM HORTOC01 #new flowfield "SalesPerson Code"
    // HEI.09 FDD-HT709 IBM NASTAA02 24.07.2019 # Ethiopia Fiscal No in PSIL
    ...
    //                                                                             Minimum Quantity
    // HEI.10  FDD-HB1609 CHG2074002 IBM BULIMC01 26.10.2020 #new field added to mark the posted free goods: 70000 - "Free Goods Posted"
    ...
    // HEI.12 CHG2109621 HT2170 IBM GAVANM01 10.06.2021 - Posting Setup for Sales Tax (Timbre), transport, free products
    //   # Added field: 50025 - Timbre applied
    // HEI.13 CHG2178940 IBM COSTES04  16.01.2023 # Add Required Freshness field on Customer Card
    //   # Add new field RFreshness Date (min)


    // BC Upgrade SHUKLP03 >>
    // HEI.02 => code is not added because of DrinkIT field "Customer DTax Group Code".
    // Field 50018 - "Suppress POS Interface" is shared with Sakshi.
    // BC Upgrade SHUKLP03 <<
    ...
    // Changed table ext name form "SalesInvoiceLine_Interface" to "SalesInvoiceLine_InterfaceFND"
    // Changed fields name from "Suppress POS Interface FND" to "Suppress POS Interface FND", "Maraki Fiscal No." to "Maraki Fiscal No. FND"
    // BC Upgrade PATELP08<<
    ...
    fields
    {
        ...
        field(50011; "Trnsprtr. RPM Damage/Loss FND"; Boolean)
        {
        ...
        field(50017; "Quantity to Return FND"; Decimal)
        {
        ...
        {
            CalcFormula = Lookup("EBM Log FND"."Maraki Fiscal No." WHERE("Document Type" = FILTER(Invoice),
        ...
        // BC Upgrade SHUKLP03 >> Interface table is used in CalcFormula so need to move in interface extesnion 

        // field(50019; "Maraki Fiscal No."; Code[30])
        // {
        //     CalcFormula = Lookup("EBM Log"."Maraki Fiscal No." where("Document Type" = FILTER(Invoice),
        //                                                               "Document No." = FIELD("Document No.")));
        //     Caption = 'Maraki Fiscal No.';
        //     Description = 'HEI.09';
        //     Editable = false;
        ...
        //BC UPGRADE KUMARR78 FDD-MTC-008 << Adding Field with New ID
        field(69999; "SalesPerson Code FND"; Code[20])
        {
        ...
        field(70000; "Free Goods Posted FND"; Boolean)
        {
            Caption = 'Free Goods Posted';
            DataClassification = ToBeClassified;
            Description = 'HEI.10';
        }
    ...
    keys
    {
        // BC Upgrade SHUKLP03 >> DrinkIT field "Is Item Charge" is used.
        // key(Key50001; "Document No.", "Attached to Line No.", "Is Item Charge")
        // {
