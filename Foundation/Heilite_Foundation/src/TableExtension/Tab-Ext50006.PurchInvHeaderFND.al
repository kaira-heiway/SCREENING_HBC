tableextension 50006 PurchInvHeaderExtFND extends "Purch. Inv. Header"
{
    // version NAVW110.0,FINXL8.00,DITW110.00.11,HEI.08
    ...
    // HEI.03 PTPGAP013 IBM PATHAA02 31.07.2017 Purchase to pay-(NPO invoice Approval/Review decision)
    //   #New field-Payment status
    // HEI.04 PURGAP05 IBM LAZARE02 31.07.2017
    //   #Extend City fields to 35; Extend Address and Address 2 fields to 60

    // HEI.05 PTPGAP041 IBM PATHAA02 20.08.2017
    // # Removed Option "OK for payment" in "Payment Status option"
    // # "Status Date"-New Field added(50004)
    // # "Payment User"-New field added (50002)
    // # Removed Code written on Status Date-On validate()
    // # Updated Code written on Payment Status-On validate()
    // # Code written on OnModify()

    // HEI.06 PTPGAP009  IBM.CHAUHB01  18/08/2017
    //   # Added new field
    //     RUID

    // HEI.07 FDD-SLSGAP001 IBM POENAB01 21.08.2017 # MDM Customer Card
    //   # New fields for MDM integration

    // HEI.08 FDD-PURGAPINT002 IBM LAZARE02 25.09.2017
    ...
    // HEI.12 FDD HT1136 CHG2088873 IBM Shankj03 16.06.2020
    //   # Added new field "License code"
    // HEI.13 FDD-HT2159 - CHG2105031 IBM NASTAA02 09.06.2021 # Centime - additional tax on VAT
    //   # New Field created: 50050 - "CAD Amount"

    // BC Upgrade SHUKLP03 >> Added in interface ext. because of dependency on table "Purch. Inv. Header Additional".
    // field "Maximo Status"
    // BC Upgrade SHUKLP03 << Added in interface ext. because of dependency on table "Purch. Inv. Header Additional".


    // BC Upgrade MISHRS14 >>
    // Blocked below property 'OptionCaptionML' due to warning in field - modify("Applies-to Doc. Type").
    // Blocked below property 'OptionCaptionML' due to warning in field - modify("Bal. Account Type").
    // Blocked global var- TempVATAmountLine as its marked for removal
    // BC Upgrade MISHRS14 <<
    // BC UPgrade BHARAD11 >>
    ...
    fields
    {
        ...
        modify("Reason Code")
        {
            ...
            var
                ...
                myInt: Integer;
            begin
                //HEI.05 IBM PATHAA02 PTPGAP041>>
                IF xRec."Reason Code" <> Rec."Reason Code" THEN
                    heinekenglobal.UpdateVendorLedgerEntry(Rec);
        ...
        modify("User ID")
        {
            ...
            //Unsupported feature: Change Editable on ""User ID"(Field 112)". Please convert manually.
        ...
        {
            CaptionML = ENU = 'Pay-to Contact No.', FRA = 'N° contact';
        }
        ...
        //Unsupported feature: CodeInsertion on ""Reason Code"(Field 73)". Please convert manually.
        ...
        */
        ...
        field(50002; "Payment User FND"; Code[50])
        {
            ...
            trigger OnValidate();
            begin
                //HEI.05 IBM PATHAA02 27.10.17>>
                "Payment User FND" := USERID;
                //HEI.05 IBM PATHAA02 28.10.17>>
            end;
        }
        field(50003; "Payment Status FND"; Option)
        {
            ...
            begin
                //HEI.05 IBM PATHAA02 PTPGAP041>>
                if xRec."Payment Status FND" <> Rec."Payment Status FND" then begin
                    CLEAR("Reason Code");
                    VALIDATE("Status Date FND");
                    VALIDATE("Payment User FND");
                    heinekenglobal.UpdateVendorLedgerEntry(Rec);
                end;
                //HEI.05 IBM PATHAA02 PTPGAP041<<
            end;
        ...
        field(50027; "Total WHT Prepd. Amt (LCY) FND"; Decimal)
        {
        ...
        {
            CalcFormula = Lookup("Purchase Additional Fields FND"."On Hold UserID" where(TableID = CONST(122),
                                                                                      "Document Type" = FILTER("Posted Invoice"),
                                                                                      "Document No." = FIELD("No.")));
        ...
        {
            CalcFormula = Lookup("Purchase Additional Fields FND"."On Hold Date" where(TableID = CONST(122),
                                                                                    "Document Type" = FILTER("Posted Invoice"),
                                                                                    "Document No." = FIELD("No.")));
        ...
        // {
        //     CalcFormula = Lookup("Purch. Inv. Header Additional"."Maximo Status" WHERE("No." = FIELD("No.")));
        ...
        field(50044; "License Code FND"; Code[20])
        {
            Description = 'HEI.12';
        ...
        field(50050; "CAD Amount FND"; Decimal)
        {
            ...
            AutoFormatType = 1;
            CalcFormula = Sum("Purch. Inv. Line"."CAD Amount FND" where("Document No." = FIELD("No.")));// BC Upgrade SHARMP16-- GAP Fit Changes
                                                                                                    // "Item Charge Type" = FIELD("Item Charge Type Filter")));//BC UPGRADE SHARMP16 drink-it field used in lookup.
        ...
        //BC UPGRADE SHARMP16 commented drink-it code begin>>
        ...
        // }
        //BC UPGRADE SHARMP16 commented drink-it code end<<
    }
    keys
    {
        // key(Key1; "Link Purch. Document No.")
        // {
        // }//BC UPGRADE SHARMP16 drink-it field used in keys.
    }
    ...
    */
    ...
    // BC Upgrade MISHRS14 >>
    // Blocked below var as its marked for removal
