...
// HEI.01 FDD RTRGAP069 - Exporting and filtering reports  IBM.NAIKH01 20.08.2017
//   # Added code on DataItem "Cash Flow Account" in Trigger "OnAfterGetRecord"
//   # Added a new parameter Amt in the function "InsertCFLineForGLAccount"

// BC Upgrade POENAB02: Original (HeiLite) report id 840 

report 50654 "SuggestWorksheetLinesHeiLite"
{
                ...
                begin
                    ...
                    if GLEntry.FindSet(false) then
                        repeat
                            ...
                            DimensionSetEntry.SetRange("Dimension Code", 'MVMT');
                            DimensionSetEntry.SetRange("Dimension Value Code", "Movement Type FND");
                            if DimensionSetEntry.FindSet() then
                                GLAmt += GLEntry.Amount;
                        until GLEntry.Next() = 0;
                    //>>HEI.01
                    ClearCircularRefData();
                    if GLAmt <> 0 then begin  //HEI.01
                        ...
                        if TempGLAccount.FindSet() then
                            repeat
                                ...
                                //InsertCFLineForGLAccount(TempGLAccount); //HEI.01
                                InsertCFLineForGLAccount(TempGLAccount, GLAmt); //HEI
...
#pragma warning restore AA0470
    ...
    local procedure InsertCFLineForGLAccount(GLAcc: Record "G/L Account"; Amt: Decimal)
    begin
        ...
        CFWorksheetLine2.Description :=
          CopyStr(
            StrSubstNo(Text013, GLAcc.Name, Format(Amt)),//HEI.01   FORMAT(GLAcc.Balance))
            1, MaxStrLen(CFWorksheetLine2.Description));
        SetCashFlowDate(CFWorksheetLine2, WorkDate());
        //CFWorksheetLine2."Amount (LCY)" := GLAcc.Balance; //HEI.01
        CFWorksheetLine2."Amount (LCY)" := Amt; //HEI.01
