...
page 50701 "PO Purch. Invoice Subform"
{
    AutoSplitKey = true;
    Caption = 'Lines';
    DelayedInsert = true;
    ...
    // version NAVW110.0.00.15140,FINXL10.01,QXL9.00.001,DITW110.00.11,HEI.14
    //BC UPGRADE SIVA Old Page ID 50084
    ...
    // HEI.03 FDDPTPGAP011 IBM HORTOC01 31.01.2018
    //   # add subpage restriction
    // HEI.04 DefectId 1466 IBM HORTOC01 07.02.2018
    //   change fields visibility- keep the standard version
    // HEI.05 PTPGAP071/RFC212 IBM HORTO01 13.07.2018
    //  Dimensions are not allowed to be changed coming from PO
    // HEI.06 Defect # 2428 IBM NASTAA02 09.08.2018 # Not able to add manual lines in PO Purchase invoices
    //   # Dimensions coming from Receipt should not be editable
    //   # Dimensions for "Manual Insert" lines should be editable except Dimenson for Trading Partner
    // HEI.07 BA-RTRGAP01 IBM NASTAA02 16.08.2018 # Bahamas VAT
    //   # New Field added - "TIN No."
    // HEI.08 HT1292 IBM SHANKJ03 04.27.2020
    //   # Field added "WHT Absorb Base"
    // HEI.09 CHG2090912 HB1641 IBM NANDIS01 01.02.2021 General Ledger Entries Description
    //   # Make visible of new field - "Additional Description"
    // HEI.10 FDD-HT2159 - CHG2105031 IBM NASTAA02 21.07.2021 # VAT Centime - Part 2 - Purchases
    //   # New Field added: "CAD Amount"
    //   # Code added on 'OnOpenPage' trigger
    // HEI.11 CHG2224401 HB3624 YADAVM09 18.04.2024 Health and Security Levy Tax
    //   # New field created #"H&S Levy Tax Amount"
    //                       #HS Posting Group
    // HEI.12 CHG2229933 HB3689 IBM SRIVAS07 03.05.2024 # SRM Reference Document Mapping - Development
    //   # Added field - "Vendor Shipment No."
    // HEI.13 CHG2224401 HB3624 YADAVM09 28.05.2024 Health and Security Levy Tax
    //   # code added to update current page
    // HEI.14 CHG2221624 HB3614 IBM SRIVAS07 15.07.2024 # Block Payment for Invoices with Price Difference higher than the tolerance
    //   # Added New Fields: - Tolerance Exceeded

    //************************************************//
    // BC UPGRADE SIVA 8/01/2026
    // SUMMARY OF CHANGES:
    //1.HEI.01,HEI.02,HEI.03,HEI.04,HEI.06,HEI.12,HEI.13,HEI.14  No Changes
    //2.HEI.10 Commented CAD related code & fields.
    //2.HEI.05 Commented Procedure CheckAllowChangeDim() logic is link to Drink IT field  Document Subtype Code_F2014421. 
    //2.HEI.07,HEI.08,HEI.09,HEI.11 Application area all will inherit from page property
    //3.DRINK-IT CUSTOM FIELDS & CodeCOMMENTED(DIT) 
    // (Range: 2013610..2035394)
    //4.Item Availability group actions(Event,Period,Location,variant,BOM Level) commented due to Method 'ShowItemAvailFromPurchLine' is marked for removal
    //and Replaced by ShowItemAvailabilityFromPurchLine in Purch. Availability Mgt
    //5.Commented Documenttoal codeunit related Logic &  Added BCHNKCustomFunction codeunit variable & Call TotalCADAmount, TotalCADIncAmount.
    ...
    // BC Upgrade SHARMP16
    // Issue: Previously compiled page was not retrieving lines during "Get Receipt Lines".
    // Resolution: Copied the base page and merged the custom code from the page that was
    // previously approved and compiled by Siva.
    layout
    {
        area(content)
        {
            repeater(PurchDetailLine)
            {
                field(Type; Rec.Type)
                {
                    ...
                    begin
                        ...
                        if (Rec.Type <> Rec.Type::"G/L Account") and (Rec.Type <> Rec.Type::" ") then
                            ERROR(this.Text001, Rec.Type)
                        else
                            Rec."Manual Insert FND" := true;
                ...
                {
                    ApplicationArea = all;
                    ToolTipML = ENU = 'Specifies the planned receipt date for the item on the purchase line.',
                                FRA = 'Spécifie la date de réception prévue pour l''article de la ligne achat.';
                ...
                field("CAD Amount"; Rec."CAD Amount FND")
                {
                    ApplicationArea = all;
                    Editable = false;
                    Visible = EnableCAD;//BC Upgrade SHARMP16
                }
                ...
                {
                    ToolTipML = ENU = 'Specifies the Health and Security Posting Group for the item or general ledger account on this line.',
                                FRA = 'Spécifie le groupe de comptabilisation santé et sécurité pour l''article ou le compte général de la ligne.';
                    ApplicationArea = all;
                    trigger OnValidate();
                    begin
                        CurrPage.UPDATE(true);//HEI.13
                    end;
                }
                ...
                field("Shortcut Dimension 1 Code"; Rec."Shortcut Dimension 1 Code")
                {
                    ...
                    trigger OnLookup(var Text: Text): Boolean
                    begin
                        this.CheckAllowChangeDim();//HEI.05
                    end;
                }
                ...
                field(ShortcutDimCode7; ShortcutDimCode[7])
                {
                    ...
                    trigger OnValidate()
                    begin
                        if Rec."Manual Insert FND" then //HEI.06
                            ERROR(this.Text50000);
                        this.CheckAllowChangeDim();//HEI.05
                ...
                {
                    ApplicationArea = all;
                    ToolTipML = ENU = 'Indicates whether the tolerance for the purchase line has been exceeded.',
                                FRA = 'Indique si la tolérance pour la ligne achat a été dépassée.';
                    Editable = false;
                }
                field("Qty. to Invoice"; Rec."Qty. to Invoice")
                {
                    ToolTipML = ENU = 'Specifies the quantity that remains to be invoiced for the item on the purchase line.';
                ...
                {
                    ApplicationArea = all;
                    ToolTipML = ENU = 'Specifies the Tax Identification Number (TIN) for the purchase line.';
            ...
            group(Control39)
            {
                ...
                group(Control15)
                {
                    ...
                    field(TotalCADAmount; TotalPurchaseLine."CAD Amount FND")
                    {
                        ...
                        Captionclass = BCHNKCustomFunction.GetTotalCADCaption(TotalPurchaseHeader."Currency Code"); //BC UPGRADE SIVA
                        Caption = 'Total CAD Amount';
                        Editable = false;
                        Visible = EnableCAD;
                        ToolTip = 'Specifies the total CAD amount.';
                    }
                    //BC UPGRADE SIVA<<
                    ...
                    field(TotalInclCAD; TotalInclCAD)
                    {
                        ApplicationArea = All;
                        AutoFormatExpression = TotalPurchaseHeader."Currency Code";
                        //CaptionClass = DocumentTotals.GetTotalInclCADCaption(Currency.Code);
                        Captionclass = BCHNKCustomFunction.GetTotalInclCADCaption(Currency.Code);//BC UPGRADE SIVA
                        Caption = 'Total Incl. CAD';
                        Editable = false;
                        Visible = EnableCAD;
                        ToolTip = 'Specifies the total amount including CAD.';
                    }
                    //BC UPGRADE SIVA
...
#pragma warning restore AA0219
    ...
    begin
        ...
        TotalInclCAD := 0;//BC Upgrade SHARMP16 CAD BEGIN<<
        CADTotalAmount := 0;
    ...
    begin
        ...
        if Rec.Type <> Rec.Type::Item then
            this.EditableDesc := true
        else
            this.EditableDesc := false;
        //PATHAA02 07.11.2017<<
        ...
        GLSetup.get;
        EnableCAD := GLSetup."Enable CAD FND";
    end;
    ...
    var
        ...
        CADTotalAmount: Decimal;//BC Upgrade SHARMP16
        BCHNKCustomFunction: Codeunit "Heineken BC Custom Functions";
        Text50000: Label 'Trading Partner dimension changes are not allowed in PO Invoices that are not manually inserted!';

        DimErr: Label 'Dimension changes are not allowed in PO Invoices';
    ...
    local procedure CheckAllowChangeDim();
    var
    ...
    begin
        ...
        PurchSetup.GET;
        PurchaseHeader.GET(Rec."Document Type", Rec."Document No.");
        if (Rec."Document Type" = Rec."Document Type"::Invoice) and (PurchSetup."PO Subtype Code FND" = PurchaseHeader."Document Subtype Code FND") and //BC Upgrade VAMSIU01 Added Document Subtype code.
           not Rec."Manual Insert FND" //HEI.06
        then
            ERROR(DimErr);
        //HEI.05 <<
    end;
