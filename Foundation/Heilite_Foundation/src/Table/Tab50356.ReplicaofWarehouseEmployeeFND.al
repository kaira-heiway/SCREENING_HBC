...
table 50356 "Warehouse Employee_DTW FND"

//BC Upgrade GUNREM01 -FDD-DTW-015 created new table for replicating warehouse employee to add zone code as primary key along with user id and location code to avoid duplicate records for same user and location code combination with different zone code.

{
    Caption = 'Warehouse Employee_DTW';
    ...
    keys
    {
        key(Key1; "User ID", "Location Code", "Zone Code")
        {
            Clustered = true;
        }
        key(Key2; Default)
