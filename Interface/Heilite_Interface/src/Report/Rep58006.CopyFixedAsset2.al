report 58006 "Copy Fixed Asset 2"
{
    /*  HEI.02 CHG2088988 IBM SHANKJ03 26/11/2020
       # uncommented code Hei.01 and Added code
      HEI.03 CHG2210794 SAHAL01 06.02.2024 Zycus - BASE HL Integration Master Dimension
        # Added Code
        # Removed Fixed Assets (CONCAT) Interface functionality due to descope
  */
    // BC Upgrade BHARDA11 >>
    // 1. The purpose of creating this report is that the base report “Copy Fixed Asset” had certain customizations in Navision. In Business Central, no event or alternative approach was available to implement the same customizations, so we had to create this report by copying the base report.
    // 2. To replace the base report with this report, we will use the 'OnSubstituteReport' substitute event.
    // BC Upgrade BHARDA11 <<
    ...
    begin
        ...
        for I := 1 to NumberofCopies do begin
            ...
            //>>HEI.02
            GeneralInterfaceSetup.GET(); //HEI.02
            DefaultDim.SETFILTER("Dimension Code", '<>%1', GeneralInterfaceSetup."Project Dimension Code");//HEI.02
