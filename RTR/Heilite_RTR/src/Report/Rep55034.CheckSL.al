report 55034 "Check SL"
{
    // version NAVW110.0,HEI.06

    // HEI.01 FDD BA-PTPGAP03 CHG2025181 IBM GAVANM01 26.08.2019 # Digital Checks Printout
    //   # New Report created based on Check BA
    //   # In Heilite Bal. Account Type should always be "G/L Account"
    // HEI.02 Defect#4442 CHG2025181 IBM BULIMc01 16.09.2019 #showing currency code in the amounts
    // HEI.03 Defect#4952 BULIMC01 IBM 23/12/2019 #code added to fill in the Document No. after "Void Check"
    // HEI.04 HT971 CHG2040699 IBM POSTOI01 06.02.2020 # add deduct the WHT Amount
    // HEI.05 FDD BA-PTPGAP03 CHG2025181 IBM GAVANM01 11.02.2020 layout changes
    // 
    ...
    //   # Commented code based on CHG2040699(Ref. :- Report 50281)
    // HEI.07 CHG2070543 IBM SHANKJ03 06.07.2020
    //   # Code commented or corrective change
    //   # currency code added in DataItem level changed from
    //     DescriptionLine[1] + ' ***' --> DescriptionLine[1] +' '+Currcode+ ' ***'
    // HEI.08 CHG2052196 IBM PANDES01 04-12-2020
    //   # Added code for Enhancement of check ledger voiding.
    // BC Upgrade KUMARR78 >>
    //
    // Report Name : Check SL
    // Report ID   : 50308
    //
    ...
    //
    // 2. Replaced deprecated Codeunit.
    //    Old:
    ...
    //              • OnPostDataItem()
    //              • ValidateDocNo()
    //         - Ensures compatibility with latest BC number series framework.
    //
    // 3. Removed obsolete NoSeriesManagement references.
    //    Old:
    //         - Codeunit NoSeriesManagement used in NAV.
    //    New:
    //         - Commented legacy reference.
    //         - Replaced with modern BC "No. Series" codeunit.
    //         - Prevents compilation issues in BC.
    // BC Upgrade KUMARR78 <<

    DefaultLayout = RDLC;
    RDLCLayout = '.\src\ReportsLayout\Check SL.rdl';
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            column(GenJnlLine_PostingDate; "Posting Date")
            {
            }
            column(DocumentNo; DocumentNo)
            {
            }
            column(GenJnlLineBankPaymentType; "Bank Payment Type")
            {
            }
            column(GenJnlLineHNKCheckNo; "HNK Check No. FND")
            {
            }
            dataitem(CheckPages; "Integer")
            {
                            ...
                            if FoundLast then begin
                                ...
                                case ApplyMethod of
                                    ...
                                    ApplyMethod::MoreLinesOneEntry:
                                        begin
                                            //HEI.04 CurrentLineAmount := GenJnlLine2.Amount;
                                            //HEI.04>>
                ...
                dataitem(PrintCheck; "Integer")
                {
                    ...
                    column(DescriptionLine1; DescriptionLine[1] + ' ' + CurrCode + ' ***')
                    {
                    }
                    ...
                    begin
                        if not TestPrint then begin
                            ...
                            // BC Upgrade MISHRS14 >> To remove warning
                            ...
                            if FoundLast then begin
                                ...
                                //DescriptionLine[1] := DescriptionLine[1] + ' ' + CurrCode;  //HEI.02
                                VoidText := '';
                            end else begin
                        ...
                        //IF ShowBNS THEN
                        //CheckAmountText2 := '********* ' + CheckAmountText + ' * ' + Currency.Code;   //commented HEI.02
                        CheckAmountText2 := '********* ' + CheckAmountText + ' * ' + CurrCode;          //HEI.02
                        //HEI.01<<
                    end;
                }
                ...
                var
                    ...
                    // NoSeriesMgt: Codeunit NoSeriesManagement; //BC Upgrade KUMARR78 Blocking As Codeunit Chaged
                    NoSeriesMgt: Codeunit "No. Series"; //BC Upgrade KUMARR78 BlocAdding king As Codeunit Chaged from NoSeriesManagement.
                ...
                begin
                    ...
                    if not TestPrint then begin
                        ...
                        if ApplyMethod <> ApplyMethod::MoreLinesOneEntry then begin
                            ...
                            //HEI.08
                            if GenJnlLine3."Bank Payment Type" = GenJnlLine3."Bank Payment Type"::"Computer Check" then //begin //Bc Upgrade YADAVM09 Warning Resolution<<
                                if GenJnlLine3."Document No." <> GenJnlLine3."HNK Check No. FND" then
                                    GenJnlLine3."Document No." := GenJnlLine3."HNK Check No. FND";
                            //end;//HEI.08 //Bc Upgrade YADAVM09 Warning Resolution<<
                            GenJnlLine3."Check Printed" := true;
                            GenJnlLine3."HNK Check No. FND" := UseCheckNo; //HEI.01
                                                                           //HEI.03<<
                            if GenJnlLine3."Document No." = '' then begin
                                ValidateDocNo();
                                //HEI.08
                                if GenJnlLine3."Bank Payment Type" = GenJnlLine3."Bank Payment Type"::"Computer Check" then begin
                                    if GenJnlLine3."Document No." <> GenJnlLine3."HNK Check No. FND" then
                                        GenJnlLine3."Document No." := GenJnlLine3."HNK Check No. FND";
                                end else//HEI.08
    ...
    requestpage
    {
        ...
        begin
            ...
            if grec_genJnlLn.FindFirst() then// begin //Bc Upgrade YADAVM09 Warning Resolution variable<<
                BankAcc2.Validate("No.", grec_genJnlLn."HNK Bank Account FND");
            //end; //Bc Upgrade YADAVM09 Warning Resolution variable<<
            //HEI.06<<
    ...
    labels
    {
        POBoxNoLbl = 'P.O. BOX'; PayToTheOrderOfLbl = 'Pay to the order of'; FirstCarrBankLbl = 'First Caribbean Int. Bank'; AddressLbl = 'PO Box 335/336, Bridge Street Castries, St. Lucia'; NoLbl = '22606410'; PayDocLbl = 'Payment Document:'; ChequeDateLbl = 'Cheque Date:'; ChequeNoLbl = 'Cheque Number:'; ChequeAmtLbl = 'Cheque Amount:'; SignatureLbl = 'Signature:'; SignaturesLbl = 'Signatures';
    }
    ...
    begin
        ...
        if GenJournalLine.FindLast() then begin //HEI.08
                                                //HEI.08
            if GenJnlLine3."Bank Payment Type" = GenJnlLine3."Bank Payment Type"::"Computer Check" then begin
                if GenJnlLine3."Document No." <> GenJnlLine3."HNK Check No. FND" then
                    GenJnlLine3."Document No." := GenJnlLine3."HNK Check No. FND";
