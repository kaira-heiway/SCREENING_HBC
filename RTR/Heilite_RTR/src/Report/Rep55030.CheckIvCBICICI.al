report 55030 "Check IvC BICICI"
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = '.\src\Reportslayout\Check IvC BICICI.rdl'; // BC Upgrade BHARDA11 ---Add layout path and Change extension RDLC to RDL.

    Caption = 'Check IvC BICICI';
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            begin
                ...
                //MntLettres2 := COPYSTR(MntLettres, 30 + STRPOS(COPYSTR(MntLettres,30),' '));
                //MntLettres := COPYSTR(MntLettres, 1, 30 + STRPOS(COPYSTR(MntLettres,30),' ')-1);

                MntLettres2 := COPYSTR(MntLettres, FindLastSpace(COPYSTR(MntLettres, 1, 36)) + 2);
                //MESSAGE(MntLettres2);
                MntLettres := COPYSTR(MntLettres, 1, FindLastSpace(COPYSTR(MntLettres, 1, 36)));
                    ...
                    case BalancingType of
                        ...
                        BalancingType::Vendor:
                            begin
                                ...
                                Vendor_Name_Concat := COPYSTR(Vendor_Name_Concat, 1, 55);
                                LastSpacePos := FindLastSpace(COPYSTR(Vendor_Name_Concat, 1, 36));
                                VendorName_1 := COPYSTR(Vendor_Name_Concat, 1, LastSpacePos);
                                if 54 - LastSpacePos < 36 then
                                    VendorName_2 := COPYSTR(Vendor_Name_Concat, LastSpacePos + 2, 54 - LastSpacePos)
                                else
    ...
    procedure SetBankAcc(BankAccNo: Code[20]);
    begin
    end;
