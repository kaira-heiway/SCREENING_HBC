report 55050 "Close Month Income Statement"
{
    // version HEI.01

    // HEI.01 FDD RTRGAP021 IBM COSTES02 27.07.2017
    //   # new report
    // HEI.02 Defect 944 IBM ISYED01 08/11/2017
    //   # added condition to enddate to be executed only if date is not 0.

    // BC Upgrade POENAB02: Original (HeiLite) report id 50012

    Caption = 'Close Month Income Statement';
    ProcessingOnly = true;
    ApplicationArea = All;
        ...
        dataitem("G/L Account"; "G/L Account")
        {
            ...
            dataitem("G/L Entry"; "G/L Entry")
            {
                ...
                begin
                    ...
                    SetRange("Posting Date", FiscalMonthStartDate, FiscMonthClosingDate);
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                ...
                {
                    Caption = 'Options';
                    field(FiscalMonthEndingDate; EndDateReq)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Fiscal Month Ending Date';
                        ToolTip = 'Specifies the last date in the closed fiscal month. This date is used to determine the closing date.';
                    ...
                    group("Close by")
                    {
                        ...
                        field(ClosePerBusUnit; ClosePerBusUnit)
                        {
                            ...
                            ApplicationArea = All;
                            ToolTip = 'Specifies that the income statement should be closed by business unit code. If this option is selected, the batch job will create separate closing entries for each business unit code that is found in the G/L entries.';
                        }
    ...
    begin
        ...
        GenJnlLine.Init();
        GenJnlLine."Posting Date" := FiscMonthClosingDate;
    ...
    var
        ...
        Window: Dialog;
        FiscalMonthStartDate: Date;
        FiscMonthClosingDate: Date;
        EndDateReq: Date;
        DocNo: Code[20];
        ...
        Text014: Label 'The fiscal month must be closed before the income statement can be closed.';
        Text015: Label 'The fiscal month does not exist.';
    ...
    begin
        ...
        if OK then begin
            FiscMonthClosingDate := ClosingDate(EndDateReq);
            OK := AccountingPeriod.Find('<');
            FiscalMonthStartDate := AccountingPeriod."Starting Date";
        end;
    ...
    begin
        ...
        if GenJnlBatch.Get(GenJnlLine."Journal Template Name", GenJnlLine."Journal Batch Name") then
            if GenJnlBatch."No. Series" <> '' then
                ...
                DocNo := NoSeries.GetNextNo(GenJnlBatch."No. Series", EndDateReq);
        // BC Upgrade POENAB02 <<
    end;
    ...
    begin
        ...
        if GLSetup."Additional Reporting Currency" <> '' then begin
            ...
            if GenJnlLine.Amount <> 0 then begin
                ...
                if DocNo = NoSeries.GetNextNo(GenJnlBatch."No. Series", EndDateReq, false) then;
                // BC Upgrade POENAB02 <<
            end;
        end else
    ...
    begin
        ...
        if DefaultDim.Find('-') then
            ...
            until (DefaultDim.Next() = 0) or (StrLen(s) > MaxStrLen(s) - StrLen(DefaultDim."No.") - StrLen(Text021) - 1);
        if s <> '' then
