...
report 53092 "Aged Accounts Receivable Copy"
{
    ...
    //Making Copy of Base Report -120.
    //Making Changes With Tag according to FDD.
    //BC UPGRADE KUMARR78 <<
    DefaultLayout = RDLC;
    RDLCLayout = '.\src\ReportsLayout\AgedAccountsReceivable.rdl';
    ...
    dataset
    {
        dataitem(Header; "Integer")
        {
            ...
            {
                RequestFilterFields = "No.", "Customer Posting Group", "CM Incl. EG Limit Filter APS";
                ...
                dataitem(OpenCustLedgEntry; "Cust. Ledger Entry")
                {
                    ...
                    begin
                        ...
                        Customer.CopyFilter("Currency Filter", "Currency Code");
                        // Customer.COPYFILTER("Item Charge Type Filter", "Item Charge Type");// BC UPGRADE KUMARR78 Blocking to Change variable in place of "Item Charge Type Filter"
                        Customer.COPYFILTER("CM Incl. EG Limit Filter APS", "CM Incl. EG. Lim. Warn APS");// BC UPGRADE KUMARR78 Adding
                        Customer.COPYFILTER("Customer Posting Group", "Customer Posting Group"); // BC UPGRADE KUMARR78 Adding
                    end;
                }
                ...
                begin
                    ...
                    if NewPagePercustomer then
                        PageGroupNo += 1;
                    TempCurrency.Reset();
                    // TempCurrency.DeleteAll();//BC UPGRADE KUMARR78 Blocking to Pass Parameter
                    TempCurrency.DeleteAll(false);//BC UPGRADE KUMARR78 Adding with Parameter

                    TempCustLedgEntry.Reset();
                    // TempCustLedgEntry.DeleteAll(); //BC UPGRADE KUMARR78 Blocking to Pass Parameter
                    TempCustLedgEntry.DeleteAll(false); //BC UPGRADE KUMARR78 Adding with Parameter
...
#pragma warning restore AA0074
    ...
    begin
        ...
        TempCustLedgEntry.Insert(false);//BC UPGRADE KUMARR78 Passing Parameter False
        if PrintAmountInLCY then begin
            Clear(TempCurrency);
            TempCurrency."Amount Rounding Precision" := GLSetup."Amount Rounding Precision";
            if TempCurrency.Insert(false) then;//BC UPGRADE KUMARR78 Passing Parameter False
            exit;
        end;
        ...
        TempCurrency.Insert(false);//BC UPGRADE KUMARR78 Passing Parameter False
    end;
