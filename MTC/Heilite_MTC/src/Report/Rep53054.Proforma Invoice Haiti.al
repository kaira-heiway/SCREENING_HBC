report 53054 "Proforma Invoice Haiti"
{
    // version HEI.08
    ...
    // *** NEW REPORT *******************************************************************************************************
    // 
    // HEI.12 FDD-HT1401 IBM BULIMC01 01.03.2021 new Report created for Haiti
    //   # Saved as from Report 50279(Proforma Invoice STD) so kept the old documentation as above under starred line, didn't delete
    // HEI.13 HT2194 CHG2107750 IBM GAVANM01 26.05.2021 – Excise Duties Haiti
    //   # code changes
    // HEI.14 INC3617037 IBM BULIMC01 26/07/2021
    ...
    // 4. Replaced Language record varible with Language codeunit because GetLanguageId() procedure is moved in Language codeunit.
    // 6. Old Report ID- 50491
    //BC Upgrade KAPOOV01  <<

    DefaultLayout = RDLC;
    //RDLCLayout = './Proforma Invoice Haiti.rdlc';//BC Upgrade KAPOOV01 Commented.
    RDLCLayout = '.\src\ReportsLayout\Proforma Invoice Haiti.rdl'; //BC Upgrade KAPOOV01-> Add layout path and change layout extension rdlc to rdl

    Caption = 'Proforma Invoice Haiti';
        ...
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(SalesHOrdNo; '') //"Sales Header"."Order No.")  //BC Upgrade KAPOOV01 Drink-IT
                    {
                    }
                    ...
                    column(SalesInvHeader_OrderDate; FORMAT("Sales Header"."Order Date", 0, '<Day,2>/<Month,2>/<Year4>'))
                    {
                    }
                    ...
                    column(SalesInvHeader_SellToNo; "Sales Header"."Sell-to Customer No.")
                    {
                    }
                    ...
                    column(InCoTerms; "Sales Header"."Shipment Method Code")
                    {
                    }
                    ...
                    dataitem("Sales Line"; "Sales Line")
                    {
                        ...
                        begin
                            ...
                            // end else if ("Item Charge Type" = "Item Charge Type"::Discount) and
                            //   ("Show Item charge on Invoice" = "Show Item charge on Invoice"::"Include in item price") then
                            //         if ItemCh.GET("No.") and not ItemCh."Transport/Shipping Cost" then   //HEI.11
                            //             CurrReport.SKIP;
                            // //HEI.07<<

                            // //HEI.12
                            // if ("Item Charge Type" = "Item Charge Type"::Discount) or ("Item Charge Type" = "Item Charge Type"::Tax) then
                            //     //IF (ItemCh.GET("No.")) AND (ItemCh."Hide Item charge on printout") THEN  //commented by HEI.13
                            //     if (ItemCh.GET("No.")) and (ItemCh."Tax Due Posting to G/L") then   //HEI.13
                            ...
                            var_Dis := "Line Discount %"; //HEI.16
                                                          ...
                                                          //                                                                         //IF NOT ItemCh."Hide Item charge on printout" THEN //HEI.12  //commented by HEI.13
                                                          //         if not ItemCh."Tax Due Posting to G/L" then //HEI.13
                                                          //                                                     //var_Dis += "Line Amount"; //HEI.09  HEI.16 commented
                    ...
                    begin
                        ...
                        if SalesInvLineAmt.FINDSET() then
                            repeat
                                //if (SalesInvLineAmt.Type <> SalesInvLineAmt.Type::"Charge (Item)") or (SalesInvLineAmt."Item Charge Type" = SalesInvLineAmt."Item Charge Type"::" ") then  //HEI.11 //BC Upgrade KAPOOV01 Drink-IT
                                if (SalesInvLineAmt.Type <> SalesInvLineAmt.Type::"Charge (Item)") then //BC Upgrade KAPOOV01 Drink-IT removed OR condition containing DRINK-IT Field
                        ...
                        if SalesInvLine.FINDSET() then
                            ...
                            //         //IF ItemCh.GET(SalesInvLine."No.") AND ItemCh."Hide Item charge on printout" THEN //HEI.12
                            //         if ItemCh.GET(SalesInvLine."No.") and ItemCh."Tax Due Posting to G/L" then //HEI.13
                            ...
                            //                     if ItemCh.GET(SalesInvLine."No.") then  //HEI.13
                            //                                                             //IF (ItemCh.GET(SalesInvLine."No.")) AND (ItemCh."Hide Item charge on printout" = FALSE) THEN BEGIN //HEI.12  //commented by HEI.13
                            //                         if not ItemCh."Tax Due Posting to G/L" then begin //HEI.13
                            //                             TotalFooterAmount[4] += SalesInvLine."Line Amount"; //HEI.12
                            //                                                                                 //TotalInvDis += SalesInvLine."Line Amount"; //HEI.14 commented
                            //                             TotalInvDis += ABS(SalesInvLine."Line Amount"); //HEI.14
                            //                         end //HEI.16
                            //                             //IF ItemCh.GET(SalesInvLine."No.") AND ItemCh."Hide Item charge on printout" THEN  //commented by HEI.13
                            //                         else //HEI.13
                            //                             TotalFooterAmount[1] += SalesInvLine."Line Amount";
                            //             //HEI.16<<
                            //         end  //HEI.11
                        ...
                        if SalesInvLine.FINDSET() then
                            repeat
                            //IF ItemCh.GET(SalesInvLine."No.") AND NOT ItemCh."Hide Item charge on printout" THEN BEGIN //HEI.12  //commented by HEI.13
                            //BC Upgrade KAPOOV01 Drink-IT Below code block is dependent on Drink-IT fields- ItemCh."Tax Due Posting to G/L" &  SalesInvLine."Free Item" >>
                            // if ItemCh.GET(SalesInvLine."No.") and not ItemCh."Tax Due Posting to G/L" then begin //HEI.13
            ...
            begin
                ...
                if (CompanyInfo."Country/Region Code" <> '') then
                    if CountryInfo.GET(CompanyInfo."Country/Region Code") then
                        // CompanyText += ', ' + CompanyInfo."Country/Region Code" + ' ' + CountryInfo.Name; //HEI.12
                        //HEI.12<<
                        CompanyText += ', ' + CountryInfo.Name + ', '; //HEI.12
                ...
                //CurrReport.LANGUAGE := Language.GetLanguageID("Language Code"); // BC Upgrade KAPOOV01 Replaced Language record varible with codeunit because GetLanguageId() procedure is moved in codeunit.
                CurrReport.LANGUAGE := LanguageG.GetLanguageID("Language Code"); // BC Upgrade KAPOOV01 Replaced Language record varible with codeunit because GetLanguageId() procedure is moved in codeunit.
                ...
                SalesInvLine.SETRANGE("Document Type", "Sales Header"."Document Type");
                SalesInvLine.SETRANGE("Document No.", "Sales Header"."No.");
                //SalesInvLine.SETFILTER("VAT %",'<>%1',0); //HEI.12
                ...
                if SalesInvLine.FINDSET() then
                    repeat
                        //BC Upgrade KAPOOV01 Drink-IT >>
                        // if not SalesInvLine."Free Item" then begin //HEI.12
                        ...
                        if TEMPAccSchedKPIBuffer.FINDFIRST() then begin
                            //IF NOT SalesInvLine."Free Item" THEN BEGIN //HEI.12 //HEI.14 commented
                            //BC Upgrade KAPOOV01 Drink-IT Below code block dependent on Drink-IT fields >>
                            // if (not SalesInvLine."Free Item") or (SalesInvLine."Item Charge Type" = SalesInvLine."Item Charge Type"::Deposit) then begin //HEI.14
                            ...
                            //         TEMPAccSchedKPIBuffer."Balance at Date Actual" := (100 / TEMPAccSchedKPIBuffer."Balance at Date Forecast") * TEMPAccSchedKPIBuffer."Net Change Budget" //HEI.12
                            //     else
                            ...
                            //IF NOT SalesInvLine."Free Item" THEN BEGIN //HEI.12 //HEI.14 commented
                            //BC Upgrade KAPOOV01 Drink-IT Below code block is dependent on Drink-IT fields- SalesInvLine."Free Item", SalesInvLine."Item Charge Type" >>
                            // if (not SalesInvLine."Free Item") or (SalesInvLine."Item Charge Type" = SalesInvLine."Item Charge Type"::Deposit) then begin //HEI.14
                            //     TEMPAccSchedKPIBuffer."Net Change Budget" += (SalesInvLine."VAT Base Amount" * SalesInvLine."VAT %") / 100;
                            //     //HEI.12<<
                            //     if TEMPAccSchedKPIBuffer."Balance at Date Forecast" <> 0 then
    ...
    begin
        ...
        CompanyInfo.CALCFIELDS("OpCo Logo FND", "OpCo Logo FND");  //HEI.12
    end;
