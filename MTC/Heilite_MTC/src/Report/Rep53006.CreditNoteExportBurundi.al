report 53006 "Credit Note Export Burundi"
{
    // version HEI.01
    ...
    //   # duplicate report created from 50388
    ...
    // 5. Blocked because of DIT Object(DocSubtypeCodeSetup."Sundry Sales Order Non Stock", DocSubtypeCodeSetup."Sundry Sales Order Stock").
    // 6. Adding ApplicationArea to the Report.
    // 7. Adding UsageCategory to the report.
    // 8. Blocking Whole Code and Adding Else condition only Due to DIT-Field Dependency.("Item Charge Type")
    // 9. Old Report ID is  50444.
    // BC Upgrade RAHUL<<

    DefaultLayout = RDLC;
    RDLCLayout = '.\src\ReportsLayout\Credit Note Export Burundi.rdl';
    ...
    dataset
    {
        dataitem("Sales Cr.Memo Header"; "Sales Cr.Memo Header")
        {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem("Sales Cr.Memo Line"; "Sales Cr.Memo Line")
                    {
                        ...
                        begin
                            ...
                            // end;
                            // BC Upgrade RAHUL << Blocking Whole Code and Adding Else condition only Due to DIT-Field Dependency.("Item Charge Type")
                            NUMLines := NUMLines - 1;
                            LinesPrinted := LinesPrinted + 1;

                            TotalInvDis += "Sales Cr.Memo Line"."Line Discount Amount";
                            // BC Upgrade RAHUL >> Adding Adding Else condition only Due to DIT-Field Dependency.("Item Charge Type")
                        end;
                    }
                    ...
                    begin
                        ...
                        // BC Upgrade RAHUL << Blocking whole lope due to DIT-Fields("Item Charge Type")
                        ...
                        //                     end else
                        //                         TotalFooterAmount[8] += SalesInvLine."Line Amount";
                        ...
                        TotalDisAmnt := TotalFooterAmount[8];
            ...
            begin
                ...
                // BC Upgrade RAHUL << Blocking Due to DIT-Field("Tax Registration No.")
                ...
                // CurrReport.LANGUAGE := Language.GetLanguageID(DocLanguage);// BC Upgrade RAHUL Blocking as GetlanguageId moved from Table to CU.
                CurrReport.Language := LanguageG.GetLanguageId(DocLanguage); // BC Upgrade RAHUL Adding GetlanguageId moved from Table to CU.
    ...
    var
        ...
        DocSubtypeCodeSetup: Record "Doc Subtype Code Setup FND"; // BC Upgrade VAMSIU01 - Added Document subtype
        GLSetup: Record "General Ledger Setup";
        // Language: Record Language; // BC Upgrade RAHUL Blocking As Function moved from Table to Codeunit.
        LanguageG: Codeunit Language;    // BC Upgrade RAHUL Adding As Function moved from Table to Codeunit.
    ...
    begin
        ...
        if "Sales Cr.Memo Header"."Document Subtype Code FND" in [DocSubtypeCodeSetup."Sundry Sales Order Non Stock", DocSubtypeCodeSetup."Sundry Sales Order Stock"] then // BC Upgrade VAMSIU01 - Added Document subtype
            ExportInvoice := false
        else begin
