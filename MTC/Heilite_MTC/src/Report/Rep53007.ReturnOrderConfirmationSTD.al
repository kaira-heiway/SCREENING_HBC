report 53007 "Return Order Confirmation STD"
{
    // version HEI.06

    // HEI.01 Report created
    // HEI.03 INC1003205 IBM HORTOC01 04.12.2018 #add new item charge discount
    // HEI.04 IBM BULIMC01 25.10.2019 # defect 4627 # code added
    //    #new variable created (lineNumberVAT)
    //    # data item TEMPAccSchedKPIBuffer_VatPercent changed
    // HEI.05 CHG2062657 HB1368 IBM GAVANM01 29.04.2020 #Correction to Invoice/Credit Note - Shipping Charge
    ...
    // HEI.07 CHG2070187 IBM GAVANM01 14.08.2020 # bug fixes
    //   # layout changes
    // HEI.08 CHG2073371 HB1589 IBM GAVANM01 28.09.2020  #St Lucia Item charges Shipping Cost not working
    //   # Item charges of type Discount and Transport/Shipping Cost = TRUE should be considered as Shipping Cost
    // BC Upgrade BHARDA11 >>
    // 1. Old Report ID is 50280
    // 2. Add layout path and Change extension RDLC to RDL.
    // 3. Remove Drink-IT Fields and relate code("Order No.",Weight,"Item Charge Type","Show Item charge on Invoice","Free Item")
    // 4. Change Language.GetLanguageID to Language codeunit.
    // 5. Add ApplicationArea Property in Report and requestpage fields.
    // BC Upgrade BHARDA11 <<

    // BC Upgrade SHUKLP03 >> Restructured DIT fields and corrected filter.
    ...
    DefaultLayout = RDLC;
    RDLCLayout = '.\src\Reportslayout\Return Order Confirmation STD.rdl'; // BC Upgrade BHARDA11 ---Add layout path and Change extension RDLC to RDL.

    Caption = 'Return Order Confirmation STD';
        ...
        dataitem("Sales Invoice Header"; "Sales Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(SalesHOrdNo; "Sales Invoice Header"."No.") // BC Upgrade SHUKLP03 
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Line")
                    {
                        ...
                        // DataItemTableView = SORTING("Document Type", "Document No.", "Line No.") WHERE(Type = FILTER(Item | Resource | "Fixed Asset" | '"Charge (Item)"')); // BC Upgrade SHUKLP03 Blocking as Wrong Expression.
                        DataItemTableView = SORTING("Document Type", "Document No.", "Line No.") WHERE(Type = FILTER(Item | Resource | "Fixed Asset" | "Charge (Item)" | "G/L Account")); // BC Upgrade SHUKLP03 << BCUPO-193
                        ...
                        begin
                            if "Sales Invoice Line".Type = "Sales Invoice Line".Type::Item then begin
                                TotalGrossWeight += "Sales Invoice Line"."Gross Weight"; // BC Upgrade SHUKLP03 ----Drink-IT Field(Weight)
                            ...
                            if not PrintDiscounts then
                                if (Type = Type::"Charge (Item)") and ("Attached Line Type 101FDW" = "Attached Line Type 101FDW"::"SPC 105FDW") then
                                    if ItemCh.GET(SalesInvoiceLine."No.") and not ItemCh."Transport/Shipping Cost FND" then  //HEI.08
                                        HideDiscount := true;
                            // BC Upgrade SHUKLP03 << ----Drink-IT Field("Item Charge Type")
                            ...
                            if (Type <> Type::"Charge (Item)") AND (Type <> Type::"G/L Account") then begin // BC Upgrade SHUKLP03 << BCUPO-193
                                ...
                                if SalesInvoiceLine.FINDSET() then
                                    repeat
                                        if ItemCh.GET(SalesInvoiceLine."No.") and not ItemCh."Transport/Shipping Cost FND" then begin  //HEI.08
                                            LineAmount += SalesInvoiceLine."Line Amount";
                                            //DiscIncluded += ABS(SalesInvoiceLine."Line Amount");  //commented by HEI.07
                                ...
                                if ("Sales Invoice Line"."Attached Line Type 101FDW" = "Sales Invoice Line"."Attached Line Type 101FDW"::"SPC 105FDW") and
                                 ("Show Item charge on Inv. FND" = "Show Item charge on Inv. FND"::"Include in item price") then
                                    if ItemCh.GET("No.") and not ItemCh."Transport/Shipping Cost FND" then   //HEI.08
                                        CurrReport.SKIP();
                            ...
                            IF SL.FINDSET() THEN
                                Repeat
                                    ...
                                    IF SL2.FINDFIRST() THEN
                                        ...
                                        // else
                                        IF ItemCh.GET(SL."No.") AND Itemch."Hide Item chrg on printout FND" AND (Itemch."Exclude/ Include in Print FND" = Itemch."Exclude/ Include in Print FND"::Free_DISC) AND (SL."Attached to Line No." = "Line No.") THEN begin // BCUPO-193
                            ...
                            ItemDiscount := 0;
                            itemDeposit := 0;
                            // BC Upgrade SHUKLP03 >> ----Drink-IT Field("Free Item","Item Charge Type")
                            ...
                            if ("Sales Invoice Line".Type = "Sales Invoice Line".Type::"Charge (Item)") and ("Sales Invoice Line"."Attached Line Type 101FDW" = "Sales Invoice Line"."Attached Line Type 101FDW"::"SPC 105FDW") then
                              //Var_typechargeItem := TRUE  //commented by HEI.08
                              //HEI.08>>
                              begin
                    ...
                    begin
                        ...
                        //SalesInvLineAmt.SETFILTER(Type,'%1|%2|%3',SalesInvLineAmt.Type::Item,SalesInvLineAmt.Type::Resource,SalesInvLineAmt.Type::"Fixed Asset"); //commented by HEI.08
                        if SalesInvLineAmt.FINDSET() then
                            repeat
                                if (SalesInvLineAmt.Type <> SalesInvLineAmt.Type::"Charge (Item)") AND (SalesInvLineAmt.Type <> SalesInvLineAmt.Type::"G/L Account") then  //HEI.08 // BCUPO-193
                        ...
                        SalesInvLine.SETFILTER(Type, '%1|%2', SalesInvLine.Type::"Charge (Item)", SalesInvLine.Type::"G/L Account");  // BCUPO-193
                        if SalesInvLine.FINDSET() then
                                        ...
                                        Begin
                                            ...
                                            if ItemCh.GET(SalesInvLine."No.") and ItemCh."Transport/Shipping Cost FND" then
                                                ...
                                                if SalesInvLine."Show Item charge on Inv. FND" <> SalesInvLine."Show Item charge on Inv. FND"::"Include in item price" then
                                                    //TotalFooterAmount[4] += ABS(SalesInvLine."Line Amount");  commented by HEI.07
                            ...
                            IF ItemChargeRec."Item Charge Type" = ItemChargeRec."Item Charge Type"::ShippingCost THEN
                              ShippingChargesAmount += SalesInvLine."Line Amount";*/
            ...
            begin
                ...
                CurrReport.LANGUAGE := Language1.GetLanguageID("Language Code"); // BC Upgrade BHARDA11 ---Change Record to codeunit::Added
                ...
                if "Currency Code" = '' then begin
                    ...
                    CurrencyCode := GLSetup."LCY Code";  //HEI.07
                end else begin
                    ...
                    CurrencyCode := "Currency Code";   //HEI.07
                end;
                ...
                if TEMPAccSchedKPIBuffer.FINDSET() then
                    repeat
                        Counter += 1;
                        // SplitVatPercent[Counter] := FORMAT(TEMPAccSchedKPIBuffer."No.") + '%'; //commented HEI.04
    ...
    labels
    {
        ...
        lblTotalQty = 'Total Quantity'; lblSalesPerson = 'Sales Person ID:'; lblUOM = 'Unit'; lblUnitPrice = 'Unit Price'; lblSaleLAmt = 'Amount Excl. VAT'; lblPageNo = 'Page No:'; lblOrderNo = 'RO Order No:'; lblInvoiceNo = 'Invoice No:'; lblVATAmt = 'Total VAT:'; lblPostDate = 'Invoice Date:'; lblDueDate = 'Due Date:'; lblPriceIncVAT = 'Price Including VAT'; lblDriver = 'Name and Driver Signature'; lblWarehouse = 'Name and Warehouse Keeper Signature'; lblSecurity = 'Name and Security Visa'; label(lblPrintDate; ENU = 'Print Date:',
    ...
    var
        ...
        // Language: Record Language; // BC Uograde BHARDA11 ::Blocked
        Language1: Codeunit Language;  // BC Uograde BHARDA11 ::Added
        ...
        Text52006: Label 'Return Order Confirmation';
        TaxAmout: Decimal;
        VATAmount: Decimal;
        DepAmount: Decimal;
        ShipAmount: Decimal;
