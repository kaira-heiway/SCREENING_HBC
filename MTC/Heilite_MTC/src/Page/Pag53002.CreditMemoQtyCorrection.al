page 53002 "Credit Memo-Qty Correction"
{
    // version HEI.01
    //BC UPGRADE SIVA Old Page ID 50137
    ...
    Caption = 'Credit Memo-Qty Correction';
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                {
                    ToolTip = 'Specifies the responsibility center that is responsible for the sales document.';
                    Editable = false;
                    Visible = false;
                }
    ...
    actions
    {
        area(navigation)
        {
            group("&Cr. Memo")
            {
                ...
                action(Statistics)
                {
                    ToolTip = 'View statistics for the sales document, such as total cost, sales, profit, invoice discount, and payment discount amounts.';
                    Caption = 'Statistics';
                    Image = Statistics;
                    Promoted = true;
                ...
                action(Approvals)
                {
                    ...
                    begin
                        //ApprovalEntries.Setfilters(DATABASE::"Sales Header", "Document Type", "No.");//BC UPGRADE SIVA Changed SetFilters to SetRecordFilters
                        ApprovalEntries.SetRecordFilters(DATABASE::"Sales Header", Rec."Document Type", Rec."No.");//BC UPGRADE SIVA Changed SetFilters to SetRecordFilters
                        ApprovalEntries.RUN();
                    end;
                }
            }
        ...
        area(processing)
        {
            ...
            group("P&osting")
            {
                ...
                action("Post &Batch")
                {
                    ToolTip = 'Schedule the posting of the document as a background task in the job queue.';
                    Caption = 'Post &Batch';
                    Ellipsis = true;
                    Image = PostBatch;
                    Promoted = true;
    ...
    begin
        ...
        if docsubtypecodesetup.GET() then begin
            docsubtypecodesetup.TESTFIELD("Credit Memo-Qty Correction");
            Rec.FILTERGROUP(2);
            Rec.SETRANGE("Document Subtype Code FND", docsubtypecodesetup."Credit Memo-Qty Correction");
            Rec.FILTERGROUP(0);
        end;
        //PATHAA02<<
        // BC UPGRADE SHUKLP03 << Added code
    ...
    begin
        //if ApplicationAreaSetup.IsFoundationEnabled then begin //BC UPGRADE SIVA Microsoft removed the check
        ...
        if Rec."Job Queue Status" = Rec."Job Queue Status"::"Scheduled for Posting" then
            ...
            // if ApplicationAreaSetup.IsFoundationEnabled then //BC UPGRADE SIVA Microsoft removed the check
