page 53009 "Gate Entry Inbound"
{
    // version HEI.04
    //BC Upgrade GUNREM01 -Old page ID 50224


    // HEI:EDD001:1:1 12/11/14 TECTURA-HKH
    //   # New Form Created for Gate Entry
    // HEI:EDD151:1:1 17/08/11 NJ
    //   # Added new field 80001 'Groped Control' [Boolean]
    // 
    ...
    // BC Upgrade PATELP08 >> 
    // # Changed the name or local procedure from OnAfterGetCurrRecord to OnAfterGetCurrRecordProcedure as Member name 'OnAfterGetCurrRecord' is only allowed on triggers.
    // BC Upgrade PATELP08 >> 
    PageType = Card;
    RefreshOnActivate = true;
    SourceTable = "Gate Entry Header FND";
    SourceTableView = WHERE("Gate Entry Type" = FILTER(Inbound),
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Grouped Control"; Rec."Grouped Control")
                {
                    ApplicationArea = all;
                    Editable = "Grouped ControlEditable";
                    ToolTip = 'Specifies the value of the Grouped Control field.';
                }
                field("Automatic Registration"; Rec."Automatic Registration")
                {
                    ApplicationArea = all;
                    ToolTip = 'Specifies the value of the Automatic Registration field.';
                }
            }
                ...
                action(CollectLines)
                {
                    ...
                    begin
                        ...
                        if WhseSetup."Allow Collect Lines FND" = true then begin
                            GateEntryOutIn.GetInboundLines(Rec."Document No.", Rec."Linked Gate Entry No.", Rec."Gate Entry Document No.")
                        end else
            ...
            group("&Register")
            {
                ...
                action(Register)
                {
                    ...
                    var
                        ...
                        WarehouseItemNo: Text[30];
                        GateEntryLines: Record "Gate Entry Line FND";
                        GateEntryItemNo: Code[20];
                        GateEntryQty: Decimal;
                    begin
                        //HEI.06
                        WarehouseReceiptHeader.SETRANGE("Gate Entry No. FND", Rec."Gate Entry Document No.");
                        //BC Upgrade GUNREM01 >> -DIT fileds
                        // WarehouseReceiptHeader.SETRANGE("Truck Code", Rec."Vehicle No.");
                        // WarehouseReceiptHeader.SETRANGE("Driver Code", Rec."Driver Code");
                        //BC Upgrade GUNREM01 << -DIT fileds
                        //BC UPGRADE KUMARR78 >> FDD-MTC-007 ++
                        WarehouseReceiptHeader.SETRANGE("Vehicle Code 101FDW", Rec."Vehicle No.");
                        WarehouseReceiptHeader.SETRANGE("Log Driver 107FDW", Rec."Driver Code");
                        //BC UPGRADE KUMARR78 << FDD-MTC-007 ++
                        ...
                        if WarehouseReceiptHeader.FINDFIRST() then begin
                            ...
                            if WarehouseReceiptLine.FINDSET() then
                                repeat
                                    if WarehouseItemNo <> WarehouseReceiptLine."Item No." then begin
                                        WarehouseQty += WarehouseReceiptLine.Quantity;
                                        WarehouseItemNo := WarehouseReceiptLine."Item No.";
                                    end else begin
                                        WarehouseQty += WarehouseReceiptLine.Quantity;
                                        WarehouseItemNo := WarehouseReceiptLine."Item No.";
                                    end;
                        ...
                        if WarehouseQty <> GateEntryQty then
                            ERROR('Gate Entry %1 is not matching with the warehouse receipt %2', GateEntryLines."Gate Entry Document No.", WarehouseReceiptHeader."No.")
                        else
                            //HEI.06
                            Rec.Register();
                    end;
    ...
    begin
        ...
        // BC Upgrade PATELP08 >> Changed the name or this local procedure from OnAfterGetCurrRecord to OnAfterGetCurrRecordProcedure as Member name 'OnAfterGetCurrRecord' is only allowed on triggers.
    ...
    var
        ...
        //[InDataSet]
        "Grouped ControlEditable": Boolean;
        //[InDataSet]
    ...
    // BC Upgrade PATELP08 >> changed procedure name from 'OnAfterGetCurrRecord' to 'OnAfterGetCurrRecordProcedure' as Member name 'OnAfterGetCurrRecord' is only allowed on triggers
