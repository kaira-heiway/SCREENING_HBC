page 53005 "Std Sales Order(Inc.FreeGoods)"
{
    // version HEI.01
    //BC UPGRADE SIVA Old Page ID 50134
    ...
    //4.Area Navigateion >> Group &Return Order >> Action Statistics is commented as it is Microsoft removal reason is new page SalesOrderStatistics added latest BC versions.
    ...
    Caption = 'Std Sales Order(Inc.FreeGoods)';
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                {
                    ToolTip = 'Specifies the approval status of the sales order.';
                }
    ...
    actions
    {
        area(navigation)
        {
            group("O&rder")
            {
                ...
                // }
                // <<BC UPGRADE SIVA<<Microsoft Removal
                //BC UPGRADE SIVA >> Microsoft update new action
                ...
                action(Approvals)
                {
                    ...
                    begin
                        ApprovalEntries.SetRecordFilters(DATABASE::"Sales Header", Rec."Document Type", Rec."No."); //BC UPGRADE SIVA Added
                        //ApprovalEntries.Setfilters(DATABASE::"Sales Header", "Document Type", "No.");//BC UPGRADE SIVA Commented
                        ApprovalEntries.RUN();
                    end;
                }
                action("Co&mments")
        ...
        area(processing)
        {
            ...
            group("&Print")
            {
                ...
                //DocPrint.PrintSalesHeaderPacking(Rec); //BC UPGRADE SIVA
                ...
                //         REPORT.RUN(2014411, true, false, SalesHeaderRecL);
                //         // >>DITW16.00.00.40 DDR DIT-715 #197
                //     end;
                // }
    ...
    begin
        ...
        if docsubtypecodesetup.GET then begin
            docsubtypecodesetup.TESTFIELD(docsubtypecodesetup."Std Sales Order(Inc.FreeGoods)");
            Rec.FILTERGROUP(2);
            Rec.SETRANGE("Document Subtype Code FND", docsubtypecodesetup."Std Sales Order(Inc.FreeGoods)");
            Rec.FILTERGROUP(0);
            //PATHAA02<<
        end;
        // BC Upgrade SHUKLP03 << ADDED CODE.
    ...
    local procedure PostSubmit(PostingCodeunitID: Integer); //BC UPGRADE SIVA Rename from Post to PostSubmit to avoid conflict with action Post
    var
        LinesInstructionMgt: Codeunit "Lines Instruction Mgt.";
    begin
        //if ApplicationAreaSetup.IsFoundationEnabled then //BC UPGRADE SIVA No need to check for microsoft foundation
