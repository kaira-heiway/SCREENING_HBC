...
page 52005 "NPO Purchase Invoices"
{
    ...
    //         # New filter on "Document Subtype Code"
    ...
    // BC UPGRADE PATELS08 >>
    // # In area(factboxes) - The "Document Attachment FactBox" has been replaced by "Doc. Attachment List factbox", thus blocking part(AttachedDocuments) and using part("Attached Documents List") instead.
    // # Action(Statistics) is blocked as the use of 'Rec.OpenDocumentStatistics();' is marked for removal. Thus instead action PurchaseStatistics is to be used.
    // # In action(PurchaseStatistics) - blocked Conditional Compilation Directives.
    // # Blocked actionref(Statistics_Promoted; Statistics) as it uses marked for removal action 'Statistics'. Instead Actionref(PurchaseStatistics_Promoted; PurchaseStatistics) is to be used.
    // BC UPGRADE PATELS08 <<
    ...
    Caption = 'NPO Purchase Invoices';
    CardPageID = "NPO Purchase Invoice";
    ...
    layout
    {
        ...
        area(factboxes)
        {
            // BC UPGRADE PATELS08 >> # The "Document Attachment FactBox" has been replaced by "Doc. Attachment List factbox", thus blocking part(AttachedDocuments) and using part("Attached Documents List") instead.
            // #if not CLEAN25
    ...
    actions
    {
        area(navigation)
        {
            group("&Invoice")
            {
                Caption = '&Invoice';
                Image = Invoice;
                // BC Upgrade PATELS08 >> # Action(Statistics) is blocked as the use of 'Rec.OpenDocumentStatistics();' is marked for removal. Thus instead action PurchaseStatistics is to be used.
                // #if not CLEAN26
                // action(Statistics)
                // {
                //     ApplicationArea = Suite;
            ...
            group("P&osting")
            {
                ...
                action(PostSelected)
                {
                    ...
                    var
                        ...
                        BeforeLimit: Label 'ENU=Document date %1 in more than 3 months old than the Posting date %2, Do you want to continue ?';
                        AfterLimit: Label 'ENU=Document date should not be more than the Posting date.';
                    begin
                        //BC Upgrade GUNREM01 >> Commented Base BC Code
                        CurrPage.SetSelectionFilter(PurchaseHeader);
        ...
        area(Promoted)
        {
            ...
            group(Category_Category4)
            {
                ...
                // BC UPGRADE PATELS08 >> # Blocked actionref(Statistics_Promoted; Statistics) as it uses marked for removal action 'Statistics'. Instead Actionref(PurchaseStatistics_Promoted; PurchaseStatistics) is to be used.
                // #if not CLEAN26
    ...
    begin
        ...
        Rec.FILTERGROUP(2);
        Rec.SETRANGE("Document Subtype Code FND", PurchasesPayablesSetup."NPO Subtype Code FND"); //BC Upgrade VAMSIU01 - Added code for Document subtype code
        Rec.FILTERGROUP(0);
        //HEI.03<<
