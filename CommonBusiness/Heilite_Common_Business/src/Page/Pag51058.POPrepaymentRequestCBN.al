page 51058 "PO Prepayment Request CBN"
{
    // version HEI.01
    ...
    //   #Code blocked on trigger OnInit() and OnNewRecord() because dependency on DrinkIT "Document Subtype Code" field.

    // HEI.06 CHG0255417 IBM.LS 15.10.2018
    //   # Code added to restrict the field modification.

    // HEI.07 CHG0255417 IBM.LS 30.10.2018
    //   # Code added to restrict the field-"Due Date" modification.

    // DrinkIT code and fields are blocked.

    // BC Upgrade SHUKLP03 >> "Document Subtype Code" code added.

    Caption = 'PO Prepayment Request';
    PageType = Document;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Buy-from Vendor No."; Rec."Buy-from Vendor No.")
                {
                    ...
                    begin
                        ...
                        // BC Upgrade SHUKLP03 >> Code blocked because dependency on DrinkIT.
            ...
            part(PurchLines; "POPrepayment Req Subform CBN")
            {
                ApplicationArea = Suite;
        ...
        area(factboxes)
        {
            ...
            // BC Upgrade SHUKLP03 >> Code blocked because dependency on DrinkIT page.
    ...
    actions
    {
        ...
        area(processing)
        {
            ...
            group("F&unctions")
            {
                ...
                // BC Upgrade SHUKLP03 >> Action blocked because dependency on DrinkIT code.
                ...
                //         //>> DITW18.00.07 DIT-770 #1804
                //     end;
                // } // BC Upgrade SHUKLP03 << Code blocked because dependency on DrinkIT code.
    ...
    begin
        ...
        Rec.FILTERGROUP(2);
        Rec.SETRANGE("Document Subtype Code FND", PurchasesPayablesSetup."PO Prepayment req. Subtype FND");
        Rec.FILTERGROUP(0);
        //HEI.03<<
        if PurchasesPayablesSetup."PO Prepayment req. Subtype FND" <> '' then
            Rec."Document Subtype Code FND" := PurchasesPayablesSetup."PO Prepayment req. Subtype FND"
        else
            ERROR(Error001);
        // BC Upgrade SHUKLP03 << "Document Subtype Code" code added.
    ...
    begin
        ...
        if PurchSetup."PO Prepayment req. Subtype FND" <> '' then
            Rec."Document Subtype Code FND" := PurchSetup."PO Prepayment req. Subtype FND"
        else
            ERROR(Error001);
