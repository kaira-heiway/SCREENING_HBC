page 51063 "Expense Claim CM Subform CBN"
{
    // version NAVW110.0,FINXL10.01,QXL9.00.001,DITW110.00.11
    ...
    //  #HEI.02 code is blocked on local procedure UpdatePaymentStatus(), because DrinkIT field "Document Subtype Code" is used. 

    //  IsFoundationArea() condition is not required anymore in business central Saas so blocked that condition.

    // DrinkIT code and fields "Has Item Charge", Collapse, "Cross-Reference No.", "GetTrackingItemNo()", "Physical Location Group Code", "Tariff No.",
    // "Approved Line Amount", RTCTotalUnit, RTCTotalLine, "Auto. Acc. Group", "Vendor DTax Group Code", "Item DTax Group Code" are blocked.
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                // BC Upgrade SHUKLP03 >> DrinkIT created fields.
                ...
                field("No."; Rec."No.")
                {
                    ...
                    begin
                        ...
                        //HEI.02>>
                        UpdatePaymentStatus(Rec);
                        if (Rec.Type <> Rec.Type::"G/L Account") then//BC Upgrade SHARMP16--PID854
                            ERROR(Text001, Rec.Type)
                        //HEI.02<<
                    end;
                }
            ...
            group(Control47)
            {
                ShowCaption = false;//BC Upgrade SHARMP16--PID854
                group(Control41)
                {
                    ShowCaption = false;//BC Upgrade SHARMP16--PID854
    ...
    begin
        ...
        PurchHeader.GET(Rec."Document Type", Rec."Document No.");//BC Upgrade SHARMP16--PID854
    ...
    begin
        ...
        Rec.ShowShortcutDimCode(ShortcutDimCode);
        //CLEAR(DocumentTotals);//BC Upgrade SHARMP16--PID854
    ...
    begin
        ...
        // // <<DITW17.10.03 DDR 12/03/2014 DIT-770 #541
        exit(Rec.Find(Which));//BC Upgrade SHARMP16--PID854
    ...
    begin
        ...
        //HEI.03<<
        if (Rec.Type <> Rec.Type::"G/L Account") then //and (Rec.Type <> Rec.Type::" ") then//BC Upgrade SHARMP16--PID854
            ERROR(Text001, Rec.Type);
        //HEI.03
    ...
    begin
        ...
        // Rec.Type := Rec.Type::Item;//BC Upgrade SHARMP16--PID854
        Rec.Type := Rec.Type::"G/L Account";//BC Upgrade SHARMP16--PID854
    ...
    // BC Upgrade MISHRS14 >>
    // Blocked the procedure as OpenItemTrackingLines is already there in standard table Purchase Line with same name
    ...
    // BC Upgrade MISHRS14 >>
    // Blocked the procedure as ShowLineComments is already there in standard table Purchase Line with same name
    ...
    begin
        // BC Upgrade SHUKLP03 >> DrinkIT code is
    ...
    // BC Upgrade SHUKLP03 >> DrinkIT function is blocked.
    ...
    begin
        ...
        if DocumentTotals.PurchaseCheckNumberOfLinesLimit(PurchHeader) then
            DocumentTotals.PurchaseRedistributeInvoiceDiscountAmounts(Rec, VATAmount, TotalPurchaseLine);
        CurrPage.UPDATE(false);//BC Upgrade SHARMP16--PID854
    end;
    ...
    // BC Upgrade SHUKLP03 >> DrinkIT created functions are blocked
