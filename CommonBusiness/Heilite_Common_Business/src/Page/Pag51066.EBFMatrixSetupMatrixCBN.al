page 51066 "EBF Matrix Setup Matrix CBN"
{
    // version HEI.01
    ...
    // BC Upgrade PATELP08 >> 
    // # Removed deprecated UpdateKey parameter from FINDSET in following 2 procedures as it is no longer supported and will cause errors in future Business Central versions
    // # 1. InsertGLAccountTmp, 2. UpdateEBF
    // BC Upgrade PATELP08 <<
    ...
    var
        ...
        CurrentDimCode: Code[100];
        EbfCombinationUpdCreate: Record "Ebf Combination FND";

    // BC Upgrade PATELP08 >> # Removed deprecated UpdateKey parameter from FINDSET as it is no longer supported and will cause errors in future Business Central versions
    local procedure InsertGLAccountTmp();
    var
    ...
    begin
        ...
        if GLAccount.FINDSET(false) then
            // BC Upgrade PATELP08 <<
            repeat
                Rec."No." := COPYSTR(GLAccount."No.", StartPosNoDigits[1], StartPosNoDigits[2]) + FilterOperator;
                if Rec.INSERT() then;
            until GLAccount.NEXT() = 0;
    end;
    // BC Upgrade PATELP08 <<

    procedure Load(MatrixColumns1: array[32] of Text[1024]; var MatrixRecords1: array[32] of Record "Dimension Value"; _ShowColumnName: Boolean; _DimCode: Code[20]);
    begin
    ...
    var
        ...
        NewDimValue: Text[100];
        Pos: Integer;
    begin
        AccountNo := Rec."No.";
        DimValueCode := MatrixRecords[ColumnID].Code;

        EbfCombination.SETFILTER("GL Account No.", Rec."No.");
        EbfCombination.SETFILTER("Dimension Code", DimCode);
        EbfCombination.SETFILTER("Dimension Value Code", '*' + DimValueCode + '*');
        if (not EbfCombination.FINDFIRST()) then begin
            if MATRIX_CellData[ColumnID] <> MATRIX_CellData[ColumnID] ::" " then begin
                EbfCombinationUpdCreate.SETFILTER("GL Account No.", Rec."No.");
                EbfCombinationUpdCreate.SETFILTER("Dimension Code", DimCode);
                EbfCombinationUpdCreate.SETFILTER("Combination Restriction", '%1', MATRIX_CellData[ColumnID]);
                if (not EbfCombinationUpdCreate.FINDFIRST()) then
                    InsertEBF(DimValueCode, ColumnID)
                else
                    UpdateEBF(DimValueCode, ColumnID, EbfCombinationUpdCreate);
                CurrPage.UPDATE();
                exit;
            end;
        end else begin
            if (EbfCombination."Combination Restriction" <> MATRIX_CellData[ColumnID]) then begin
                Pos := STRPOS(EbfCombination."Dimension Value Code", DimValueCode);
                if (Pos = 1) then
                    NewDimValue := DELSTR(EbfCombination."Dimension Value Code", Pos, 9)
                else
                    NewDimValue := DELSTR(EbfCombination."Dimension Value Code", Pos - 1, 8);
                if (STRLEN(NewDimValue) = 0) then
                    EbfCombination.DELETE(true)
                else begin
                    EbfCombination.RENAME(Rec."No.", DimCode, NewDimValue);
                    EbfCombinationUpdCreate.SETFILTER("GL Account No.", Rec."No.");
                    EbfCombinationUpdCreate.SETFILTER("Dimension Code", DimCode);
                    EbfCombinationUpdCreate.SETFILTER("Combination Restriction", '%1', MATRIX_CellData[ColumnID]);
                    if (not EbfCombinationUpdCreate.FINDFIRST()) then
    ...
    local procedure InsertEBF(DimValueCode: Code[20]; ColumnId: Integer);
    begin
        EbfCombinationUpdCreate."GL Account No." := Rec."No.";
        EbfCombinationUpdCreate."Dimension Code" := DimCode;
        EbfCombinationUpdCreate."Dimension Value Code" := DimValueCode;
        EbfCombinationUpdCreate."Combination Restriction" := MATRIX_CellData[ColumnId];
        EbfCombinationUpdCreate.INSERT(true);
    end;

    // BC Upgrade PATELP08 >> # Removed deprecated UpdateKey parameter from FINDSET as it is no longer supported and will cause errors in future Business Central versions
    local procedure UpdateEBF(DimValueCode: Code[20]; ColumnId: Integer; EbfComb: Record "Ebf Combination FND");
    var
        NewDimValue: Code[100];
        lEbfCombination: Record "Ebf Combination FND";
        FieldLenght: Integer;
    begin
        FieldLenght := MAXSTRLEN(EbfComb."Dimension Value Code");
        lEbfCombination.RESET();
        lEbfCombination.SETFILTER("GL Account No.", EbfComb."GL Account No.");
        lEbfCombination.SETFILTER("Dimension Code", DimCode);
        lEbfCombination.SETFILTER("Combination Restriction", '%1', MATRIX_CellData[ColumnId]);
        // BC Upgrade PATELP08 >> # Removed deprecated UpdateKey parameter from FINDSET as it is no longer supported and will cause errors in future Business Central versions
        //if EbfCombinationUpdCreate.FINDSET(false, false) then
        if EbfCombinationUpdCreate.FINDSET(false) then
            // BC Upgrade PATELP08 <<
            repeat
                NewDimValue := EbfCombinationUpdCreate."Dimension Value Code" + '|' + DimValueCode;
                if STRLEN(NewDimValue) <= FieldLenght then begin
                    EbfCombinationUpdCreate.RENAME(Rec."No.", DimCode, NewDimValue);
                    exit;
                end;
            until EbfCombinationUpdCreate.NEXT() = 0;
        InsertEBF(DimValueCode, ColumnId)
    end;
    // BC Upgrade PATELP08 <<
}
