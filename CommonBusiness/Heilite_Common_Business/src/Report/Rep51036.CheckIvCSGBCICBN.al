report 51036 "Check IvC SGBCI CBN"
{
    ...
    // BC Upgrade KUMARS145 Nav ID Report 50281 "Check IvC SGBCI"

    DefaultLayout = RDLC;
    RDLCLayout = '.\src\ReportsLayout\Check IvC SGBCI.rdl';
    Caption = 'Check IvC SGBCI';
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
                ...
                dataitem(PrintCheck; "Integer")
                {
                    ...
                    begin
                        if not TestPrint then begin
                            ...
                            // end;// BC upgrade KUMARS145 Removed 'with' statement as it is deprecated.
                        end else begin
                            // with GenJnlLine do begin // BC upgrade KUMARS145 Removed 'with' statement as it is deprecated.
                            CheckLedgEntry.Init();
                            CheckLedgEntry."Bank Account No." := BankAcc2."No.";
            ...
            begin
                ...
                MntLettres2 := CopyStr(MntLettres, 45 + StrPos(CopyStr(MntLettres, 45), ' '));
                MntLettres := CopyStr(MntLettres, 1, 45 + StrPos(CopyStr(MntLettres, 45), ' ') - 1);
    ...
    labels
    {
        ...
        label(VendorNoLB; ENU = 'Vendor No', FRA = 'Vendor No')
        label(VendorNameLB; ENU = 'Vendor Name', FRA = 'NOM')
        label(VendorBankLB; ENU = 'Vendor Bank', FRA = 'N° DE COMPTE BANCAIRE')
        label(VendorAddressLB; ENU = 'Address', FRA = 'ADRESSE')
        label(InvoiceNoLB; ENU = 'Invoice No', FRA = 'N° DES FACTURES')
        ExternalDocNoLB = 'External Doc No.';
        label(AmountLB; ENU = 'Amount', FRA = 'MONTANT')
        label(DeviseLB; ENU = 'Devise', FRA = 'Devise')
        label(AmountLCYLB; ENU = 'Amount LCY', FRA = 'MONTANT LCY')
        label(DueDateLB; ENU = 'Due Date', FRA = 'DATE D''ECHEANCE')
        label(BankLB; ENU = 'Bank', FRA = 'Banque')
        TotalLbl = 'Total by Vendor';
        SignatureLB = 'Signature';
    }
    ...
    begin
        ...
        GenJnlLine3.SetFilter("Parent Line No.",'=%1',0);
    ...
    procedure InitializeRequest(BankAccPar: Code[20]; LastCheckNoPar: Code[20]; NewOneCheckPrVend: Boolean; NewReprintChecks: Boolean; NewTestPrint: Boolean; NewPreprintedStub: Boolean);
    begin
        if BankAccPar <> '' then
            if BankAcc2.Get(BankAccPar) then begin
                UseCheckNo := LastCheckNoPar;
    ...
    local procedure ExchangeAmt(PostingDate: Date; CurrencyCode: Code[10]; CurrencyCode2Par: Code[10]; Amount: Decimal) Amount2: Decimal;
    begin
        if (CurrencyCode <> '') and (CurrencyCode2Par = '') then
            Amount2 :=
            ...
            if (CurrencyCode = '') and (CurrencyCode2Par <> '') then
                Amount2 :=
                  CurrencyExchangeRate.ExchangeAmtFCYToLCY(
                    PostingDate, CurrencyCode2Par, Amount, CurrencyExchangeRate.ExchangeRate(PostingDate, CurrencyCode2Par))
            else
                if (CurrencyCode <> '') and (CurrencyCode2Par <> '') and (CurrencyCode <> CurrencyCode2Par) then
                    Amount2 := CurrencyExchangeRate.ExchangeAmtFCYToFCY(PostingDate, CurrencyCode2Par, CurrencyCode, Amount)
                else
                    Amount2 := Amount;
    end;
    ...
    begin
        if GenJnlLine."Currency Code" = '' then
            ...
            CurrencyRec.Get(GenJnlLine."Currency Code");
            CurrencyRec.TestField("Amount Rounding Precision");
        end;
        exit(1 / CurrencyRec."Amount Rounding Precision");
    end;

    procedure MontantEnTexte(var strprix: Text[250]; prix: Decimal);
    var
        entiereLocal: Decimal;
        decimalLocal: Decimal;
        nbreLocal: Decimal;
        millionLocal: Text;
        milleLocal: Text;
        centLocal: Text;
        nbre1Local: Decimal;
    begin
        entiereLocal := Round(prix, 1, '<');
        decimalLocal := Round((prix - entiereLocal) * 1000, 1, '<');

        nbreLocal := entiereLocal;
        //Chèque.FormatNumTexte(strprix,nbreLocal);

        millionLocal := '';
        milleLocal := '';
        centLocal := '';

        nbre1Local := nbreLocal div 1000000;
        if nbre1Local <> 0 then begin
            Centaine(millionLocal, nbre1Local);
            millionLocal := millionLocal + ' million';
        end;

        nbreLocal := nbreLocal mod 1000000;
        nbre1Local := nbreLocal div 1000;
        if nbre1Local <> 0 then begin
            Centaine(milleLocal, nbre1Local);
            if milleLocal <> 'un' then
                milleLocal := milleLocal + ' mille'
            else
                milleLocal := 'mille'
        end;

        nbreLocal := nbreLocal mod 1000;

        if nbreLocal <> 0 then begin
            Centaine(centLocal, nbreLocal);
        end;

        if millionLocal <> '' then
            strprix := millionLocal;
        if ((milleLocal <> '') and (strprix <> '')) then
            strprix := strprix + ' ' + milleLocal
        else
            strprix := strprix + milleLocal;
        if ((centLocal <> '') and (strprix <> '')) then
            strprix := strprix + ' ' + centLocal
        else
            strprix := strprix + centLocal;

        if entiereLocal > 1 then
            strprix := strprix + ' FCFA ';
        if entiereLocal = 1 then
            strprix := strprix + ' dinar';

        centLocal := '';
        if decimalLocal <> 0 then begin
            Centaine(centLocal, decimalLocal);
            if strprix <> '' then
                strprix := strprix + ' ' + centLocal
            else
                strprix := strprix + centLocal;
            if decimalLocal = 1 then
    ...
    procedure Centaine(var chaine: Text[250]; iPar: Integer);
    var
        k: Integer;
    begin
        k := iPar div 100;
    ...
    procedure Dizaine(var chaine: Text[250]; iPar: Integer);
    var
        k: Integer;
        l: Integer;
        chaine1Local: Text;
    begin
        if iPar > 16 then begin
            k := iPar div 10;
            chaine1Local := '';
            case k of
                1:
                ...
                3:
                    chaine1Local := 'trente';
                4:
                    chaine1Local := 'quarante';
                5:
                    chaine1Local := 'cinquante';
                6:
                    chaine1Local := 'soixante';
                7:
                    chaine1Local := 'soixante';
                8:
                    chaine1Local := 'quatre vingt';
                9:
                    chaine1Local := 'quatre vingt';
            end;
            if ((chaine1Local <> '') and (chaine <> '')) then
                chaine1Local := ' ' + chaine1Local;
            chaine := chaine + chaine1Local;
            l := k;
            if ((k = 7) or (k = 9)) then
                k := (iPar mod 10) + 10
            else
    ...
    procedure "Unité"(var chaine: Text[250]; iPar: Integer);
    var
        chaine1Local: Text;
    begin
        chaine1Local := '';
    ...
    begin
        ...
        if decimalLocal <> 0 then begin
            if strprix <> '' then
                strprix := strprix + ' ' + Format(decimalLocal)
            else
                strprix := strprix + Format(decimalLocal);
            if decimalLocal = 1 then
                strprix := strprix + ' millime'
    ...
    var
        iLocal: Integer;
    begin
        for iLocal := 1 to StrLen(String) do begin
            if String[StrLen(String) + 1 - iLocal] = ' ' then
                exit(StrLen(String) - iLocal);
        end;
    end;
}
