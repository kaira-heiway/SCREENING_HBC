report 51051 "Sales Invoice Burundi CBN"
{
    // version HEI.01
    ...
    // 
    // HEI.03 Defect #5024 IBM POSTOI01 11.12.2019 # Burundi FAT
    //   #correct the subtotal calculations
    //   #VATAmount field from layout expression was modified from Fields!VATAmount.Value+Fields!SalesDiscount.Value->Fields!VATAmount.Value
    //   #InvTotalAmt field from layout expression was modified from :
    //   =IIF(
    // Fields!SalesHIncVAT.Value="Yes",
    // (Fields!DepositAmount.Value + Fields!SubTotal.Value+Fields!TaxAmount.Value + Fields!BaseMarginAmt.Value - Fields!TotalInvDis.Value),
    // Fields!SubTotal.Value+Fields!TaxAmount.Value+Fields!VATAmount.Value + Fields!DepositAmount.Value+Fields!ShippingChargesAmount.Value+Fields!MarkupChargesAmount.Value + Fields!BaseMarginAmt.Value - Fields!TotalInvDis.Value)
    // to
    // =IIF(
    // Fields!SalesHIncVAT.Value="Yes",
    // (Fields!DepositAmount.Value + Fields!SubTotal.Value+Fields!TaxAmount.Value + Fields!BaseMarginAmt.Value ),
    // Fields!SubTotal.Value+Fields!TaxAmount.Value+Fields!VATAmount.Value + Fields!DepositAmount.Value+Fields!ShippingChargesAmount.Value+Fields!MarkupChargesAmount.Value + Fields!BaseMarginAmt.Value)
    // HEI.04 CHG2060791 IBM SHANKJ03
    //   # Defect 5444
    //   # Sub Total Incl VAT layout Expression was Modified From
    //     Fields!SubTotal.Value+Fields!TaxAmount.Value+Fields!FreeVATAmount.Value+Fields!ShippingChargesAmount.Value+Fields!MarkupChargesAmount.Value + Fields!BaseMarginAmt.Value
    //     to
    //     Fields!SubTotal.Value+Fields!TaxAmount.Value+Fields!FreeVATAmount.Value+Fields!ShippingChargesAmount.Value+Fields!MarkupChargesAmount.Value
    //   # Total to be paid layou expression was modified  from
    //     Fields!SubTotal.Value+Fields!TaxAmount.Value+Fields!FreeVATAmount.Value + Fields!DepositAmount.Value+Fields!ShippingChargesAmount.Value+Fields!MarkupChargesAmount.Value + Fields!BaseMarginAmt.Value
    //     to
    //     Fields!SubTotal.Value+Fields!TaxAmount.Value+Fields!FreeVATAmount.Value + Fields!DepositAmount.Value+Fields!ShippingChargesAmount.Value+Fields!MarkupChargesAmount.Value
    // HEI.05 CHG2064677 IBM SHANKJ03 28.05.2020
    //   # Layout changes
    // HEI.06 CHG2069329 IBM SAMANR01 08.07.2020
    ...
    // HEI.07 CHG2144396 IBM GHOSHS05 01.02.2022 Added PrintingTime to show proper time in both Webclient and RTC
    //********************************************************
    //BC UPGRADE PATHAA02 15-Dec-25
    //GetlanguageId moved from Table to CU.
    //Code conditions related DIT fields are commented but need to uncomment back once Aptean Ext/Code is merged.


    DefaultLayout = RDLC;
    //RDLCLayout = '.\src\Sales Invoice Burundi.rdl';//BC UPGRADE PATHAA02
    RDLCLayout = '.\src\Reportslayout\Sales Invoice Burundi.rdl';//BC UPGRADE PATHAA02  
    CaptionML = ENU = 'Sales Invoice Burundi',
                FRA = 'Facture Vente Burundi';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            // column(CompanyInfo_BankName2; CompanyInfo."Bank Name 2")//BC UPGRADE-DIT(F2029610)
            // {
            // }

            column(CompanyInfo_BankName2; CompanyInfo."Name 2")
            {
            }

            // column(CompanyInfo_BankAcc2; CompanyInfo."Bank Account No. 2")//BC UPGRADE PATHAA02-DIT(F2029611)
            // {
            // } 

            // column(CompanyInfo_IBAN2; CompanyInfo."IBAN 2") //BC UPGRADE PATHAA02-DIT(F2029612)
            // {
            // }
            // column(CompanyInfo_Swift2; CompanyInfo."SWIFT Code 2") //BC UPGRADE PATHAA02-DIT(F2029613)
            // {
            // }
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        begin
                            if "Sales Invoice Line".Type = "Sales Invoice Line".Type::Item then begin
                                //TotalGrossWeight += "Sales Invoice Line".Weight; //BC UPGRADE PATHAA02-DIT(F2014080)
                            ...
                            ItemDiscount := 0;
                            itemDeposit := 0;
                            //if not "Sales Invoice Line"."Free Item" then //BC UPGRADE PATHAA02-DIT(2013826)
                            TotalInvDis := "Sales Invoice Line"."Line Discount Amount";

                            //BC UPGRADE PATHAA02-code commented for ("Item Charge Type")>>
                            /*
                            ...
                            */
                            ...
                            //BC UPGRADE PATHAA02-DIT(F2035394-"Show Item charge on Invoice")-commented code>>
                            /*
                            //HEI.03>>
                            ...
                            */
                            ...
                            SalesChargeLine.SETRANGE("Document No.", "Sales Invoice Line"."Document No.");
                            SalesChargeLine.SETRANGE(Type, "Sales Invoice Line".Type::"Charge (Item)");
                            // SalesChargeLine.SETRANGE("Item Charge Type", "Sales Invoice Line"."Item Charge Type"::Discount);//BC UPGRADE PATHAA02-code commented for ("Item Charge Type")
                            SalesChargeLine.SETRANGE("Attached to Line No.", "Sales Invoice Line"."Line No.");
                            if SalesChargeLine.findset() then begin
                                repeat
                                    if SalesChargeLine."No." <> 'S_SHIP' then begin //HEI.03
                                        IsDiscount := true;
                            ...
                            SalesChargeLine.SETRANGE("Document No.", "Sales Invoice Line"."Document No.");
                            SalesChargeLine.SETRANGE(Type, "Sales Invoice Line".Type::"Charge (Item)");
                            //SalesChargeLine.SETRANGE("Item Charge Type", "Sales Invoice Line"."Item Charge Type"::Deposit);//BC UPGRADE PATHAA02-code commented for ("Item Charge Type")
                    ...
                    begin
                        ...
                        //HEI.03 SalesInvLineAmt.SETFILTER(Type,'%1|%2|%3',SalesInvLineAmt.Type::Item,SalesInvLineAmt.Type::Resource,SalesInvLineAmt.Type::"Fixed Asset");
                        //HEI.03>>
                        ...
                        if SalesInvLineAmt.findset() then
                            repeat
                            //BC UPGRADE PATHAA02-code commented for ("Item Charge Type") & "Show Item charge on Invoice">>
                            /*
                        ...
                        if SalesInvLine.findset() then
                            repeat
                                    ...
                                    SalesInvLine."Item Charge Type"::Discount:
                                        begin
                                            case SalesInvLine."No." of
                                                ...
                                                'S_SHIP':
                                                    begin
                                                        TotalFooterAmount[6] += SalesInvLine.Amount;
                                                        ShippingChargesAmount := TotalFooterAmount[6] //HEI.03
                                                    end;
                                            end;
                                        end;
                                ...
                                */
                                ...
                                if ItemChargeRec."Shipping Cost BPM FND" then
                                    ShippingChargesAmount += SalesInvLine.Amount;
                            //#BCUP0-RTR-BPM Item Charges BC Upgrade KAIRAR01 <<
                            //HEi.04 >>
                            /*
                        ...
                        if SalesInvLine.findset() then
                            repeat
                            //BC UPGRADE PATHAA02-DIT("Item Charge Type"&"Show Item charge on Invoice") code commented>>
                            /*
            ...
            begin
                ...
                //BC UPGRADE PATHAA02-DIT-F2013726>>
                ...
                */
                ...
                // SalesInvLine_1.SETRANGE("Free Item", false); //BC UPGRADE PATHAA02-F2013826
    ...
    labels
    {
        ...
        label(LblVatRegistrationNo; ENU = 'Vat Registration No:',
                                   FRA = 'Numéro d''identificayion fiscal:',
    ...
    var
        ...
        //LanguageG: Record Language; //BC UPGRADE PATHAA02
        ...
        Text52005: TextConst ENU = 'Subtotal %1 Excl. VAT:', FRA = 'Sous-total %1 hors TVA', ENG = 'Subtotal %1 Excl. VAT:';
        ...
        TxtInvCurr: TextConst ENU = 'Amount in BIF:', FRA = 'Montant en BIF:', ENG = 'Amount in BIF:';
    ...
    begin
        ...
        if Customer.GET("Sales Invoice Header"."Bill-to Customer No.") then
            if Customer."Account Group FND" in ['Y005', 'Y006', 'Y008'] then
                ICInvoice := true;
        //BC UPGRADE SHUKLP03 >> SIH-"Document Subtype Code" 
