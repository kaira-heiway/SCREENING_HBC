pageextension 51199 SalesJournalExtCBN extends "Sales Journal"
{
    ...
    //BC Upgrade GUNREM01 Added code in onopenpage(), OnAfterGetRecord(),OnAfterGetCurrRecord()
    ...
    layout
    {
        modify(CurrentJnlBatchName)
        {
            ...
            ToolTipML = ENU = 'Specifies the batch name on the sales journal.', FRA = 'Spécifie le nom de la feuille vente.';
        }
        modify("Posting Date")
        {
        ...
        {
            ToolTipML = ENU = 'Specifies the customer''s product type to link transactions made for this customer with the appropriate general ledger account according to the general posting setup.', FRA = 'Spécifie le type de produit du client pour lier les transactions effectuées pour ce client au compte général approprié en fonction des paramètres de validation généraux.';
        }
        ...
        modify("Profit (LCY)")
        {
            ToolTipML = ENU = 'Specifies the line''s profit amount if you are using this journal line for an invoice.', FRA = 'Indique le montant de la marge de la ligne si vous utilisez cette ligne feuille pour une facture.';
        }
        ...
        {
            CaptionML = ENU = 'Balance', FRA = 'Solde';
            ToolTipML = ENU = 'Specifies the balance that has accumulated in the sales journal on the line where the cursor is.', FRA = 'Indique le solde du document dans la feuille vente sur la ligne où se trouve le pointeur de la souris.';
        }
        ...
        {
            CaptionML = ENU = 'Total Balance', FRA = 'Solde final';
            ToolTipML = ENU = 'Specifies the total balance in the sales journal.', FRA = 'Indique le solde final de la feuille vente.';
        }
    ...
    //BC Uprade GUNREM01 >> added code from onittrigger
    trigger OnOpenPage()
    begin
        //HEI.01>>
    ...
    //BC Uprade GUNREM01 << added code from onittrigger
    //BC Uprade GUNREM01 >> added code from onaftergetrecord
    trigger OnAfterGetRecord()
    begin
        EnableActnIfTemplateNtBlck := Rec.EnableActionIfTemplateNtBlock();  //HEI.01
    end;
    //BC Uprade GUNREM01 << added code from onaftergetrecord
    //BC Uprade GUNREM01 << added code from onaftergetCurrecord
